...
class Test(BaseTest):
    ...
    def run(test):
        ...
        # toggle_off_rts(test, checkstep=1)
        test.expect(socket_service.dstl_get_service().dstl_load_profile())
        # EF_Soft_reset(test, checkstep=1)
...
def toggle_off_rts(test, checkstep=1):
    global alternative_step
    if checkstep == alternative_step:
...
def EF_Soft_reset(test, checkstep=1):
    global alternative_step
    if checkstep == alternative_step:
        toggle_on_rts(test)
