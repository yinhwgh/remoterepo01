...
def uc_init_module(test, start_step,alternative_step=0):
    ...
    if start_step <= 4:
        ...
        if alternative_step == 8:
            toggle_off_rts(test)
        test.log.step('[Init]:[NF-08]: Enables HW flow control (AT\Q3) Error result code (numeric) Automatic response to Network requests (ATS0)')
...
def NF_04_wait_for_sysstart(test):
    return test.expect(test.dut.at1.wait_for('^SYSSTART', 34))
...
def NF_14_enable_urc(test):
    ...
    result = result & test.expect(test.dut.at1.send_and_verify('AT^SCFG="GPRS/AutoAttach","enabled"', 'OK\r\n', timeout=5, handle_errors=True))
    result = result & test.expect(test.dut.at1.send_and_verify('AT^SCFG="GPIO/mode/FSR","std"', 'OK\r\n', timeout=5, handle_errors=True))
...
def NF_15_setup_network(test):
    ...
    result = result & test.expect(test.dut.at1.send_and_verify('AT+CGDCONT=1,"IP","","",0,0', 'OK\r\n', timeout=5, handle_errors=True))
...
def NF_16_setup_short_message(test):
    ...
    result = result & test.expect(test.dut.at1.send_and_verify('AT+CMGL="ALL"', 'OK\r\n', timeout=5, handle_errors=True))
    return result
