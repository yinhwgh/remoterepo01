...
def NF_01_config_ip_servive_connection_sics(test, alternative_step):
    if alternative_step == 1:
        return False
    result = True
    result = result & test.expect(test.dut.at1.send_and_verify('AT+CGDCONT=1,"IP","cmnet"', 'OK\r\n', timeout=5, handle_errors=True))
...
def NF_03_open_internet_service(test, alternative_step):
    if alternative_step == 3:
        return False
    result = True
    result = result & test.expect(test.dut.at1.send_and_verify('AT^SISO=0', 'SISW', wait_after_send=10, handle_errors=True))
