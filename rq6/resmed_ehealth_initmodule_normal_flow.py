...
def uc_init_module(test, start_step, alternative_step=0):
    ...
    if start_step <= 11:
        ...
        if get_reinit_flag() is True:
            return True
        if alternative_step == 18:
            toggle_off_rts(test)
        test.log.step('[Init]:[NF-18]: check the network registration')
        retry(test, NF_18_read_real_time, AF_B_SW_restart, 3)
        if get_reinit_flag() is True:
            return True
        if alternative_step == 19:
            toggle_off_rts(test)
        test.log.step('[Init]:[NF-19]: check the network registration')
        retry(test, NF_19_report_server_certificate, AF_B_SW_restart, 3)
        if get_reinit_flag() is True:
            return True
        if alternative_step == 20:
            toggle_off_rts(test)
        test.log.step('[Init]:[NF-20]: check the network registration')
        retry(test, NF_20_init_server_certificate_store, AF_B_SW_restart, 3)
...
def NF_15_setup_network(test):
    ...
    result = result & test.expect(test.dut.at1.send_and_verify('AT+COPS=3,2', 'OK\r\n', timeout=5, handle_errors=True))
    # AT+COPS=3,2
    return result
...
def NF_18_read_real_time(test):
    return test.expect(test.dut.at1.send_and_verify('AT+CCLK?', 'OK\r\n', timeout=5, handle_errors=True))


def NF_19_report_server_certificate(test):
    return test.expect(test.dut.at1.send_and_verify('AT^SIND="is_cert",1', 'OK\r\n', timeout=5, handle_errors=True))


def NF_20_init_server_certificate_store(test):
    r1 = test.expect(
