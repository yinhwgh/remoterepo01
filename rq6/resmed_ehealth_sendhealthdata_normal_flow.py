...
run_all = False  # False for normal flow,which does not need retry.True for exception flow.
...
def uc_send_healthdata(test, start_step, alternative_step=0):
    ...
    if start_step == 1:
        ...
        #      toggle_off_rts(test)
        test.log.step(
            '[SendHealthData][NF-01]Configures the IP service connection profile 0 with an inactive timeout set to 90 seconds and a private APN using AT^SICS.')
    ...
    if start_step <= 2:
        ...
        #     toggle_off_rts(test)
        test.log.step(
            '[SendHealthData][NF-02]closes the internet service profile (ID 0) and configures this service profile as a secure TCP socket in non-transparent mode (server authentication only).')
    ...
    if start_step <= 3:
        ...
        #     toggle_off_rts(test)
        test.log.step(
            '[SendHealthData][NF-04]sends health data (~100k-1MByte) to the receiving server in 1500Bytes chunks and checks the connection status with AT^SISI=0 in between.')
...
def NF_01_config_ip_servive_connection_sics(test, alternative_step):
    ...
    result = result & test.expect(
        test.dut.at1.send_and_verify('AT+CGDCONT=1,"IP","cmnet"', 'OK\r\n', timeout=5, handle_errors=True))
...
def NF_02_config_ip_servive_connection_siss(test, alternative_step):
    ...
    result = result & test.expect(
        test.dut.at1.send_and_verify('AT^SISS=0,"srvType","socket"', 'OK\r\n', timeout=5, handle_errors=True))
    result = result & test.expect(
        test.dut.at1.send_and_verify('AT^SISS=0,"conId","1"', 'OK\r\n', timeout=5, handle_errors=True))
    # global ip_address, port_number
    result = result & test.expect(
        test.dut.at1.send_and_verify(f'AT^SISS=0,"address","socktcps://{test.ip_address}:{test.port_number}"', 'OK\r\n',
                                     timeout=5, handle_errors=True))
    return result
...
def NF_03_open_internet_service(test, alternative_step):
    ...
    result = True
    result = result & test.expect(
        test.dut.at1.send_and_verify('AT^SISO=0', 'SISW', wait_after_send=10, handle_errors=True))
...
def NF_04_send_health_data(test, alternative_step):
    ...
    for i in range(10):
        result = result & test.expect(
            test.dut.at1.send_and_verify('AT^SISW=0,1500', '^SISW: 0,1500,0\r\n', timeout=5, handle_errors=True))
