...
# location: Beijing
# TC0107138.001 - SmartMeter_sendAlertMessage_first_SMS_configure_error_exceptionalFlowA_update
# Hints:
...
class Test(BaseTest):
    ...
    def run(test):
        ...
        send_alert_message_first_SMS_configure_error_exceptional_flowA_update(test)

    def cleanup(test):
        pass

def send_alert_message_first_SMS_configure_error_exceptional_flowA_update(test):
    ...
    test.log.info('*** send_alert_message_first_SMS_configure_error_exceptional_flowA_update begin ***')
    ...
    test.log.step('NF-07: Configure the SMS parameters. --- Trigger error')
    test.log.step('NF-08: Shut down the module via at^smso.')
    test.log.step('NF-09: Wait a security time about 5 seconds.')
    test.log.step('NF-10: Boot up the module via the ignition line and re-initialized module.')
    test.dut.dstl_collect_result('NF-07: Configure the SMS parameters.\n'
                                 'NF-08: Shut down the module via at^smso.\n'
                                 'NF-09: Wait a security time about 5 seconds.\n'
                                 'NF-10: Boot up the module via the ignition line and re-initialized module.',
                                 smart_meter_init_module_normal_flow.retry(test, smart_meter_send_alert_message_alternativeFlowB_update.configure_SMS,
                                                                           smart_meter_init_module_normal_flow.AF_A_restart,
                                                                           3))

    test.dut.at1.reconfigure({"rtscts": False})
    test.log.step('NF-11: Repeat step1-7 --- NF-01: Check for listener mode.')
    smart_meter_init_module_normal_flow.set_listener_mode(False)
    if smart_meter_init_module_normal_flow.check_listener_mode(test):
        if smart_meter_init_module_normal_flow.check_dcd_line(test) is True:
            return
            ...
            smart_meter_init_module_normal_flow.close_listener(test)
            test.dut.dstl_collect_result('NF-11: Repeat step1-7 --- NF-01: Check for listener mode', False)
    test.dut.dstl_collect_result('NF-11: Repeat step1-7 --- NF-01: Check for listener mode', True)

    test.log.step('NF-11: Repeat step1-7 --- NF-02: Check if tampering attempt.')
    tamperingAttempt_status = False
    test.dut.dstl_collect_result('NF-11: Repeat step1-7 --- NF-02: Check if tampering attempt.', True)

    test.log.step('NF-11: Repeat step1-7 --- NF-03: Send the module to sleep.')
    enable_power_saving_mode(test)
    test.expect(test.dut.at1.send_and_verify("AT\Q 0", ".*OK|.*0"))
    test.expect(test.dut.at1.send_and_verify('AT^SCFG="tcp/withurcs","on"', ".*OK|.*0"))
    test.dut.dstl_collect_result('NF-11: Repeat step1-7 --- NF-03: Send the module to sleep.',
                                 smart_meter_init_module_normal_flow.retry(test, check_enter_sleep,
                                                                           smart_meter_init_module_normal_flow.AF_A_restart,
                                                                           3))

    test.log.step('NF-11: Repeat step1-7 --- NF-04:  Check if really power loss or only a glitch for 9 seconds.')
    time.sleep(9)
    test.dut.dstl_collect_result('NF-11: Repeat step1-7 --- NF-04:  Check if really power loss or only a glitch for 9 seconds.', True)

    test.log.step('NF-11: Repeat step1-7 --- NF-05:  Wake up the module.')
    test.dut.dstl_collect_result('NF-11: Repeat step1-7 --- NF-05:  Wake up the module.',
                                 smart_meter_init_module_normal_flow.retry(test,
                                                                           smart_meter_send_alert_message_normal_flow_update.wake_up_module,
                                                                           smart_meter_init_module_normal_flow.AF_A_restart,
                                                                           3))
    test.log.step('NF-11: Repeat step1-7 --- NF-06: disables power saving.')
    test.dut.dstl_collect_result('NF-11: Repeat step1-7 --- NF-06: disables power saving.',
                                 smart_meter_init_module_normal_flow.retry(test, disable_power_saving_mode,
                                                                           smart_meter_init_module_normal_flow.AF_A_restart,
                                                                           3))

    test.log.step('NF-11: Repeat step1-7 --- NF-07: Configure the SMS parameters.')
    test.dut.dstl_collect_result('NF-11: Repeat step1-7 --- NF-07: Configure the SMS parameters.',
                                 smart_meter_init_module_normal_flow.retry(test, smart_meter_send_alert_message_alternativeFlowB_update.configure_SMS,
                                                                           smart_meter_init_module_normal_flow.AF_A_restart,
                                                                           3))

    test.log.step('NF-12:  Send the alert as short message.')
    test.dut.dstl_collect_result('NF-12: Configure the first UDP socket',smart_meter_init_module_normal_flow.retry(test, smart_meter_send_alert_message_alternativeFlowB_update.send_short_message1,
                                                                          smart_meter_init_module_normal_flow.AF_A_restart,3))

    test.log.step('NF-13: Check if power loss.')
    power_loss = False
    test.dut.dstl_collect_result('NF-13: Check if power loss.', True)

    test.log.step('NF-14: Configure the second SMS.')
    test.dut.dstl_collect_result('NF-14: Configure a second SMS.',smart_meter_init_module_normal_flow.retry(test, smart_meter_send_alert_message_alternativeFlowB_update.configure_SMS,
                                                                          smart_meter_init_module_normal_flow.AF_A_restart,3))
    ...
    test.log.info('*** send_alert_message_first_SMS_configure_error_exceptional_flowA_update end ***')
    return True
