...
# location: Beijing
# TC0107136.001 - SmartMeter_sendAlertMessage_normalFlow_update
# Hints:
...
class Test(BaseTest):
    ...
    def run(test):
        ...
        send_alert_message_normal_flow_update(test)

    def cleanup(test):
        pass

def send_alert_message_normal_flow_update(test):
    global tamperingAttempt_status, glitch_status, power_loss, ipVersion
    tamperingAttempt_status = False
    glitch_status = False
    power_loss = False
    ipVersion = "6"
    test.log.info('*** send_alert_message_normal_flow_update begin ***')
    ...
    test.dut.dstl_collect_result('NF-05: Wake up the module.',
                                 smart_meter_init_module_normal_flow.retry(test, wake_up_module,
    ...
    test.dut.dstl_collect_result('NF-08: Configure the first UDP socket', smart_meter_init_module_normal_flow.retry(test, configure_udp1_socket,
                                                                          smart_meter_init_module_normal_flow.AF_A_restart,3))

    test.log.step('NF-09: Open UDP socket and send the alert as UDP packet.')
    test.dut.dstl_collect_result('NF-09: Open UDP socket and send the alert as UDP packet', smart_meter_init_module_normal_flow.retry(test, open_udp_socket_and_send_data,
                                                                          smart_meter_init_module_normal_flow.AF_A_restart,3))
    ...
    test.dut.dstl_collect_result('NF-11: Close the UDP socket', smart_meter_init_module_normal_flow.retry(test, close_udp_socket,
                                                                          smart_meter_init_module_normal_flow.AF_A_restart,3))
    ...
    test.dut.dstl_collect_result('NF-13: Configure a second UDP socket.', smart_meter_init_module_normal_flow.retry(test, configure_udp2_socket,
                                                                          smart_meter_init_module_normal_flow.AF_A_restart,3))

    test.log.step('NF-14: Open UDP socket and send the alert as UDP packet to a second server.')
    ...
    test.log.step('NF-15: Wait for 5 secs for reception.')
    test.sleep(5)
    test.dut.dstl_collect_result('NF-15: Wait for 5 secs for reception.', True)

    test.log.step('NF-16: Close the second UDP socket..')
    test.dut.dstl_collect_result('NF-16: Close the second UDP socket.',
                                 smart_meter_init_module_normal_flow.retry(test, close_udp_socket,
                                                                     smart_meter_init_module_normal_flow.AF_A_restart,
                                                                     3))
    test.expect(test.dut.at1.send_and_verify('AT^SCFG="tcp/withurcs","off"', ".*OK|.*0"))
    
    test.log.step('NF-17: Check for listener mode')
    if smart_meter_init_module_normal_flow.get_run_all() == True:
        ...
        if smart_meter_init_module_normal_flow.check_listener_mode(test) == True:
            test.dut.dstl_collect_result(
                'NF-17: Check for listener mode during duration test--It should be true and got to UC ProvideDataConnection',
                True)
            return True
        ...
        if smart_meter_init_module_normal_flow.check_listener_mode(test):
            if smart_meter_init_module_normal_flow.check_dcd_line(test) is True:
                ...
                smart_meter_init_module_normal_flow.close_listener(test)
                test.dut.dstl_collect_result('NF-17: Check for listener mode with DCD checking and closing listener',
                                             True)
        else:
            test.dut.dstl_collect_result(
                'NF-17: Check for listener mode without operation of DCD checking and closing listener', True)

    test.dut.dstl_print_results()
    test.log.info('*** send_alert_message_normal_flow_update end ***')
    return True
...
def wake_up_module(test):
    ...
    while result == False:
        test.dut.at1.send("AT", wait_after_send=0.0, timeout=0)
        result = test.expect(test.dut.at1.send_and_verify("AT", ".*OK|.*0|.*", timeout=0))
        if retry_times == 3:
            test.log.error('Wakeup module for 3 times failed!')
            break
        retry_times += 1
    return result
...
def close_udp_socket(test):
    result = True
    result = result & test.expect(test.dut.at1.send_and_verify("AT^SISC=0", "(^|\r|\r\n|\n)(.*?OK|0)(\r|\r\n|\n|$)"))
#    udp_client.dstl_get_service().dstl_close_service_profile()
    return result
