...
# location: Beijing
# TC0107165.001 - SmartMeter_sendAlertMessage_wakeup_error_exceptionalFlowA_update
# Hints:
...
class Test(BaseTest):
    ...
    def run(test):
        ...
        send_alert_message_wakeup_error_exceptional_flowA_update(test)

    def cleanup(test):
        pass

def send_alert_message_wakeup_error_exceptional_flowA_update(test):

    global power_save_period, power_save_wakeup
    power_save_period = 300
    ...
    power_loss = False
    ipVersion = "4"
    test.log.info('*** send_alert_message_wakeup_error_exceptional_flowA_update begin ***')
    ...
    test.expect(test.dut.at1.send_and_verify("AT\Q 3", ".*OK|.*0"))
    ...
    test.log.step('NF-05: Wake up the module.--- Trigger error')
    test.log.step('NF-06: Shut down the module via at^smso.')
    test.log.step('NF-07: Wait a security time about 5 seconds.')
    test.log.step('NF-08: Boot up the module via the ignition line and re-initialized module.')
    test.dut.dstl_collect_result('NF-05:  Wake up the module.\n'
                                 'NF-06: Shut down the module via at^smso.\n'
                                 'NF-07: Wait a security time about 5 seconds.\n'
                                 'NF-08: Boot up the module via the ignition line and re-initialized module.',
    ...
    test.dut.at1.reconfigure({"rtscts": False})
    test.log.step('NF-09: Repeat step1-5 --- NF-01: Check for listener mode.')
    smart_meter_init_module_normal_flow.set_listener_mode(False)
    if smart_meter_init_module_normal_flow.check_listener_mode(test):
        if smart_meter_init_module_normal_flow.check_dcd_line(test) is True:
            return
            ...
            smart_meter_init_module_normal_flow.close_listener(test)
            test.dut.dstl_collect_result('NF-09: Repeat step1-5 --- NF-01: Check for listener mode', False)
    test.dut.dstl_collect_result('NF-09: Repeat step1-5 --- NF-01: Check for listener mode', True)

    test.log.step('NF-09: Repeat step1-5 --- NF-02: Check if tampering attempt.')
    tamperingAttempt_status = False
    test.dut.dstl_collect_result('NF-09: Repeat step1-5 --- NF-02: Check if tampering attempt.', True)

    test.log.step('NF-09: Repeat step1-5 --- NF-03: Send the module to sleep.')
    enable_power_saving_mode(test)
    test.expect(test.dut.at1.send_and_verify("AT\Q 0", ".*OK|.*0"))
    test.expect(test.dut.at1.send_and_verify('AT^SCFG="tcp/withurcs","on"', ".*OK|.*0"))
    test.dut.dstl_collect_result('NF-09: Repeat step1-5 --- NF-03: Send the module to sleep.', smart_meter_init_module_normal_flow.retry(test, check_enter_sleep,
                                              smart_meter_init_module_normal_flow.AF_A_restart, 3))

    test.log.step('NF-09: Repeat step1-5 --- NF-04:  Check if really power loss or only a glitch for 9 seconds.')
    time.sleep(9)
    test.dut.dstl_collect_result('NF-09: Repeat step1-5 --- NF-04:  Check if really power loss or only a glitch for 9 seconds.', True)

    test.log.step('NF-09: Repeat step1-5 --- NF-05:  Wake up the module.')
    test.dut.dstl_collect_result('NF-05:  Wake up the module.',
    ...
    test.log.info('*** send_alert_message_wakeup_error_exceptional_flowA_update end ***')
...
def wake_up_module(test):
    ...
    while result == False:
        ...
        result = test.expect(test.dut.at1.send_and_verify("AT", ".*OK.*", timeout=0))
        test.log.error('Wakeup module failed!')
        break
    return result
