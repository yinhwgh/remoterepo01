...
step_list3 = ['NF_21_trigger_fw_swap', 'NF_22_wait_for_systart', 'NF_23_check_firmware',
              'NF_24_init_module', 'NF_25_config_server_type_http', 'NF_26_config_service_conid_http',
              'NF_27_config_service_address_http', 'NF_28_config_service_hcmethod_post',
              'NF_29_config_service_parameter',
              'NF_30_activate_internet_connection_http', 'NF_31_open_internet_service_http', 'NF_32_wait_for_sisw',
              'NF_33_send_tracking_data', 'NF_34_check_data', 'NF_35_close_internet_service_http']


class Test(BaseTest):
    """
     TC0107884.001 - Trakmate_TrackingUnit_UpdateModule_EF02
...
@step_with_error_handle(max_retry=1, step_if_error=eval('error_handle_1'))
def NF_25_config_server_type_http(test, force_abnormal_flow=False):
    return test.expect(test.dut.at1.send_and_verify('AT^SISS=2,"srvType","http"', handle_errors=True))


@step_with_error_handle(max_retry=1, step_if_error=eval('error_handle_1'))
def NF_26_config_service_conid_http(test, force_abnormal_flow=False):
    return test.expect(test.dut.at1.send_and_verify('AT^SISS=2,"conid","1"', handle_errors=True))


@step_with_error_handle(max_retry=1, step_if_error=eval('error_handle_1'))
def NF_27_config_service_address_http(test, force_abnormal_flow=False):
    return test.expect(
        test.dut.at1.send_and_verify(f'AT^SISS=2,"address","{address_2}"', handle_errors=True))


@step_with_error_handle(max_retry=1, step_if_error=eval('error_handle_1'))
def NF_28_config_service_hcmethod_post(test, force_abnormal_flow=False):
    r1 = test.expect(test.dut.at1.send_and_verify('AT^SISS=2,"cmd","post"', handle_errors=True))
    r2 = test.expect(test.dut.at1.send_and_verify('AT^SISS=2,"hcContLen","500"', handle_errors=True))
    return r1 & r2


@step_with_error_handle(max_retry=1, step_if_error=eval('error_handle_1'))
def NF_29_config_service_parameter(test, force_abnormal_flow=False):
    return test.expect(
        test.dut.at1.send_and_verify('AT^SISS=2,"hcProp","Content-Type: \'text/plain\'"', handle_errors=True))


@step_with_error_handle(max_retry=1, step_if_error=eval('error_handle_1'))
def NF_30_activate_internet_connection_http(test, force_abnormal_flow=False):
    return test.expect(test.dut.at1.send_and_verify("AT^SICA=1,1", handle_errors=True))


@step_with_error_handle(max_retry=1, step_if_error=eval('error_handle_1'))
def NF_31_open_internet_service_http(test, force_abnormal_flow=False):
    return test.expect(test.dut.at1.send_and_verify("AT^SISO=2", handle_errors=True))


@step_with_error_handle(max_retry=1, step_if_error=eval('error_handle_1'))
def NF_32_wait_for_sisw(test, force_abnormal_flow=False):
    return test.expect(test.dut.at1.wait_for('^SISW: 2,1', 20))


@step_with_error_handle(max_retry=1, step_if_error=eval('error_handle_1'))
def NF_33_send_tracking_data(test, force_abnormal_flow=False):
    send_data = dstl_generate_data(500)
    r1 = test.expect(
        test.dut.at1.send_and_verify("AT^SISW=2,500", expect='^SISW: 2,500,0', wait_for='^SISW', handle_errors=True))
    r2 = test.expect(test.dut.at1.send_and_verify(send_data, expect='^SISW: 2,2', wait_for='^SISW', handle_errors=True))
    return r1 & r2


@step_with_error_handle(max_retry=1, step_if_error=eval('error_handle_1'))
def NF_34_check_data(test, force_abnormal_flow=False):
    return test.expect(test.dut.at1.send_and_verify("AT^SISI=2", expect='500', handle_errors=True))


@step_with_error_handle(max_retry=1, step_if_error=eval('error_handle_1'))
def NF_35_close_internet_service_http(test, force_abnormal_flow=False):
    global stop_run
    result = test.expect(test.dut.at1.send_and_verify("AT^SISC=2", handle_errors=True))
    if result:
        stop_run = True
    return result
