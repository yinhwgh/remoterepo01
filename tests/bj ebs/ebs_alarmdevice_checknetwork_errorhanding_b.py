...
class Test(BaseTest):
    """
       TC0108051.001 is intended to test the normal flow of init module according to RQ6000095.001 EBS - AlarmDevice - CheckNetwork
    """
...
def main_process(test):
    ...
    test.dut.at1.reconfigure({"baudrate": 230400})
    test.sleep(5)
    test.log.info("******HW reset the module******")
    ...
    test.dut.at1.wait_for('SYSSTART', 60)
    test.dut.devboard.send_and_verify("MC:EMERG=10", ".*")
    test.dut.at1.wait_for('SYSSTART', 60)
    test.expect(test.dut.dstl_enter_pin())
