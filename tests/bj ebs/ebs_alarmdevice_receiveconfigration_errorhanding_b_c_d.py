...
class Test(BaseTest):
    """
       TC0108059.001 is intended to test the normal flow of init module according to RQ6000095.001 EBS - AlarmDevice - CheckNetwork
    """
...
def main_process(test):
    ...
    test.expect(test.dut.at1.send_and_verify('AT^SISC=1', ".*OK.*"), msg="Start Error Handong C")
    test.dut.devboard.send_and_verify("MC:CCIN=0", ".*OK.*")
    test.sleep(10)
    test.log.step("a. Check service status")
    test.expect(test.dut.at1.send_and_verify('AT^SISI=1', ".*OK.*"))
    ...
    test.dut.devboard.send_and_verify("MC:CCIN=1", ".*OK.*")
    test.expect(test.dut.at1.send_and_verify('AT^SISO=1', ".*OK.*"), msg="Start Error Handong B")
    test.dut.devboard.send_and_verify("MC:CCIN=0", ".*OK.*")
    test.sleep(10)
    test.log.step("a. Check service status")
    test.expect(test.dut.at1.send_and_verify('AT^SISI=1', ".*OK.*"))
    ...
    test.dut.devboard.send_and_verify("MC:CCIN=0", ".*OK.*")
    test.sleep(10)
    test.log.info("******Soft reset the module******")
    ...
    test.sleep(5)
    test.expect(test.dut.dstl_enter_pin())
    test.log.info("******Return to the normal flow ******")
    test.log.step("6. Set echo, ATE0 and expect return OK.")
    test.expect(test.dut.at1.send_and_verify("ATE0", ".*OK.*"))
    test.log.step("7. Set error messages format with AT+CMEE=1.")
    test.expect(test.dut.at1.send_and_verify("AT+CMEE=1", ".*OK.*"))
    ...
    for i in range(5):
        ...
        test.expect(test.dut.at1.send_and_verify('AT^SISR=1,511', ".*OK.*"))
        test.sleep(30)
    test.log.step("b. Read new settings from the server.")
    test.expect(test.dut.at1.send_and_verify('AT^SISR=1,200', ".*OK.*"))
    test.log.step("c. Change application settings to new values.")
    test.log.step("d. Go to error handling B (soft reset with new settings).")
