...
class Test(BaseTest):
    ...
    """
     TC0108055.001 - EBS_AlarmDevice_ReceiveConfigration_NF
    """
...
def check_network(test):
    ...
    test.socket = SocketProfile(test.dut, srv_profile_id="1", con_id="2", alphabet="1", protocol="tcp", host=host_address,
                                port="6666", tcp_mr="10", tcp_ot="6000")
...
def main_process(test):
    ...
    test.log.step('2. Read new settings from the server')
    ...
    test.expect(test.socket.dstl_get_service().dstl_read_all_data
                (req_read_length=200))
    test.log.step('4.Change application settings to new values, this maybe customer s action.')
    test.log.step('5.Go to error handling B (soft reset with new settings). TC0108051.001 ')
