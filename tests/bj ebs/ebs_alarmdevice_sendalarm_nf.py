...
senddata = "!@#$%^&*()1234567890abcdefghijklmnopqrstuvwxyz!@#$%^&*()1234567890abcdefghijklmnopqrstuvwxyz!@#$%^&*()1234567890abcdefghijklmnopqrstuvwxyz!@#$%^&*()1234567890abcdefghijklmnopqrstuvwxyz!@#$%^&*()1234567890abcdefghijklmnopqrstuvwxyz!@#$%^&*()1234567890abcdefghijklmnopqrstuvwxyz!@#$%^&*()1234567890abcdefghijklmnopqrstuvwxyz!@#$%^&*()1234567890abcdefghijklmnopqrstuvwxyz!@#$%^&*()1234567890abcdefghijklmnopqrstuvwxyz!@#$%^&*()1234567890abcdefghijklmnopqrstuvwxyz!@#$%^&*()1234567890abcdefghijklmnopqrstuvwxyz!@#$dd"
sendSMS = "0891683110900405F001000D91685141502435F70008080054004500530054"
...
def ebs_sendalarm(test, start_step, alternative_step=0):
    ...
    if alternative_step == 2:
        toggle_off_rts(test)
    test.log.step('[SendAlarm][NF-02]Send alarm')
    retry(test, NF_02_send_alarm, EF_B, 3)
    if alternative_step == 3:
        toggle_off_rts(test)
    test.log.step('[SendAlarm][NF-03]Check if heartbeat was acknowledged')
    if start_step == 2:
        retry(test, NF_03_check_if_heartbeat_was_acknowledged, EF_C, 1)
    else:
        retry(test, NF_03_check_if_heartbeat_was_acknowledged, EF_B, 3)
    if alternative_step == 4:
        toggle_off_rts(test)
    test.log.step('[SendAlarm][NF-04]Set SMS parameters')
    retry(test, NF_04_set_sms_parameters, EF_B, 3)
    test.log.step('[SendAlarm][NF-05]Set coding')
    NF_05_set_coding_gsm(test)
    test.log.step('[SendAlarm][NF-06]Send SMS with report')
    NF_06_send_sms_with_report(test)
    test.log.step('Test again with UCS2 coding')
    test.log.step('[SendAlarm][NF-05]Set coding')
    NF_05_set_coding_ucs2(test)
    test.log.step('[SendAlarm][NF-06]Send SMS with report')
    NF_06_send_sms_with_report(test)
    return True
...
def NF_02_send_alarm(test):
    result = True
    result = result & test.expect(
        test.dut.at1.send_and_verify('AT^SISW=1,511', '^SISW: 1,511,0\r\n', timeout=5, handle_errors=True))
...
def NF_04_set_sms_parameters(test):
    return test.expect(test.dut.at1.send_and_verify('AT+CMGF=0', 'OK', timeout=5, handle_errors=True))


def NF_05_set_coding_gsm(test):
    return test.expect(test.dut.at1.send_and_verify('AT+CSCS=GSM', 'OK', timeout=5, handle_errors=True))


def NF_05_set_coding_ucs2(test):
    return test.expect(test.dut.at1.send_and_verify('AT+CSCS=UCS2', 'OK', timeout=5, handle_errors=True))


def NF_06_send_sms_with_report(test):
    result = True
    result = result & test.expect(test.dut.at1.send_and_verify('AT+CMGS=22', '>'))
    result = result & test.expect(
        test.dut.at1.send_and_verify(sendSMS, end="\u001A", expect=r"\+CMGS: \d{1,3}(.*OK.*|.*0.*)", timeout=20))
    return result
...
def EF_C(test):
    ...
    test.expect(ebs_check_network(test, 4))
    return True
...
def ebs_check_network(test, start_step, alternative_step=0):
    ...
    if start_step <= 4:
        ...
        test.expect(test.dut.at1.send_and_verify('AT+CGDCONT=2,IPV6,"radius"', ".*OK.*"))
    ...
    if start_step <= 57:
        ...
        test.expect(test.dut.at1.send_and_verify('AT^SISS=1,address,"socktcp://[2001:1b77:a401:1f::3A]:6666"', ".*OK.*"))
