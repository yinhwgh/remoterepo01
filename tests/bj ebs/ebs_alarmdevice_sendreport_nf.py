...
senddata = "!@#$%^&*()1234567890abcdefghijklmnopqrstuvwxyz!@#$%^&*()1234567890abcdefghijklmnopqrstuvwxyz!@#$%^&*()1234567890abcdefghijklmnopqrstuvwxyz!@#$%^&*()1234567890abcdefghijklmnopqrstuvwxyz!@#$%^&*()1234567890abcdefghijklmnopqrstuvwxyz!@#$%^&*()1234567890abcde"
...
def ebs_sendreport(test, start_step, alternative_step=0):
    if alternative_step == 1:
        toggle_off_rts(test)
    test.log.step('[SendReport][NF-01]Check service status')
    if start_step == 2:
    ...
    if alternative_step == 2:
        toggle_off_rts(test)
    test.log.step('[SendReport][NF-02]Send report')
    retry(test, NF_02_send_report, EF_B, 3)
    if alternative_step == 3:
        toggle_off_rts(test)
    test.log.step('[SendReport][NF-03]Check if heartbeat was acknowledged')
    if start_step == 2:
    ...
    if alternative_step == 4:
        toggle_off_rts(test)
    test.log.step('[SendReport][NF-04]Go to Check network step 81Check service status')
...
def NF_02_send_report(test):
    result = True
    result = result & test.expect(
        test.dut.at1.send_and_verify('AT^SISW=1,255', '^SISW: 1,255,0\r\n', timeout=5, handle_errors=True))
