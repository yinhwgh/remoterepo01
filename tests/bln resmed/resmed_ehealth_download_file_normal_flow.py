...
class Test(BaseTest):
    """
     TC0107796.001-Resmed_eHealth_DownloadFile_NormalFlow
     modified by yinhw add main_process function to called
    """
    ...
    def run(test):
        ...
        #
        # test.log.step("10. Client: Send data 1500 bytes.")
        # test.expect(socket_service.dstl_get_service().dstl_send_sisw_command_and_data
        #             (1500, repetitions=10))
        # test.log.info('Server send 1500 data ')
        # test.ssl_server.dstl_send_data_from_ssh_server(data=server_send_data,
        #                                                ssh_server_property='ssh_server')
        # test.expect(socket_service.dstl_get_urc().dstl_is_sisr_urc_appeared(1))
        # test.log.info('Server send 1500 data*9 ')
        # for i in range(1, 10):
        #     test.ssl_server.dstl_send_data_from_ssh_server(data=server_send_data,
        #                                                    ssh_server_property='ssh_server')
        # test.sleep(5)
        ...
        # test.sleep(10)
        # rx_data = socket_service.dstl_get_parser().dstl_get_service_data_counter(counter="rx")
        # tx_data = socket_service.dstl_get_parser().dstl_get_service_data_counter(counter="tx")
        # test.expect(rx_data == tx_data)
        #
        # test.log.step("12. Check if server response is correct.")
