...
def kill_ssh_process(test, ssh_server_property, process_name, indexes_to_kill=[0,1,2,3]):
        ...
        try:
            for index in indexes_to_kill:
                if 'grep' in server_response[index]:
                    ...
                    process_id = \
                    server_response[index].replace('  ', ' ').replace('  ', ' ').replace('  ', ' ').split(' ')[1]
                    print("-----------process_id : ", process_id)
                    ssh_server.send_and_receive('sudo kill {}'.format(process_id))
        except IndexError:
            dstl.log.warn('Incorrect response - cannot parse process ID.')
        ssh_server.send_and_receive('lsof -i:50433')
        server_response = ssh_server.last_response.split('\n')
        server_response.remove('')
        try:
            process_id = server_response[1].replace('  ', ' ').replace('  ', ' ').replace('  ', ' ').split(' ')[1]
            print("-----------process_id : ", process_id)
            ssh_server.send_and_receive('sudo kill {}'.format(process_id))
        except IndexError:
            dstl.log.warn('Incorrect response - cannot parse process ID.')
...
def main_process(test):
    ...
    # test.ssl_server.dstl_kill_ssh_process(ssh_server_property="ssh_server_5", process_name='openssl-1.0')
    kill_ssh_process(test, ssh_server_property="ssh_server_2", process_name='openssl-1.0')
