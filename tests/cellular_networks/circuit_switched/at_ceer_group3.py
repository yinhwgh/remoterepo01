...
class Test(BaseTest):
    '''
        TC0000186.001 - IndCeerRelCauseGroup3
        Check presentation of  indicators "ceer" if parameter <CeerRelCauseGroup> is set to 3
        3 = CS internal cause
    ...
    def run(test):
        ...
        test.expect(test.dut.at1.send_and_verify('AT^SIND="ceer",1,3', expect='.*OK.*'))
        test.expect(test.dut.at1.send_and_verify('AT^SIND="ceer",2', expect='.*3*'))
        ...
        test.expect(test.r1.at1.send_and_verify('AT+CHUP','', wait_for=''))
        test.expect(test.dut.at1.wait_for('.*\+CIEV: ceer,2,"Normal call clearing".*||.*\+CIEV: ceer,2,"Invalid/incomplete number".*'))
        ...
        test.expect(test.dut.at1.send_and_verify('AT^SBLK', expect='.*OK*'))
        test.expect(test.dut.at1.send_and_verify('AT+CLCK="FD",1,"2345"', expect='.*OK*'))
        ...
        test.expect(test.dut.at1.send_and_verify('AT+CLCK="FD",0,"2345"', expect='.*OK*'))
