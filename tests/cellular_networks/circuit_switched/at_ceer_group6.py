...
class Test(BaseTest):
    '''
        ...
        Check presentation of  indicators "ceer" if parameter <CeerRelCauseGroup> is set to 6
        6 = CS internal cause
    ...
    def run(test):
        ...
        test.expect(test.dut.at1.send_and_verify('AT^SIND="ceer",1,6', expect='.*OK.*'))
        test.expect(test.dut.at1.send_and_verify('AT^SIND="ceer",2', expect='.*6*'))
