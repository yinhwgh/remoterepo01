...
class Test(BaseTest):
    """
    TC0091845.001 - TpAtDNAndAtChupBasic
    Intention: This procedure provides basic tests for the test and write command of ATD<n>.
    Subscriber: 2
    """
    ...
    def setup(test):
        ...
        if test.r1_phone_num_regexpr.startswith('0'):
            ...
            test.expect(False, msg="phone number of r1 is invalid: {}".format(test.r1_phone_num))
        pass
    ...
    def run(test):
        ...
        test.expect(test.dut.at1.send_and_verify('atd>1;', 'ERROR|NO CARRIER|NO DIALTONE',
        ...
        test.expect(test.dut.at1.send_and_verify('atd>123;', 'ERROR'))
        test.expect(test.dut.at1.send_and_verify('atd>12;', 'ERROR'))
        pass
    ...
    def func_test(test, pb_storage, index):
        ...
        test.expect(test.dut.at1.send_and_verify('atd>{};'.format(index), '', wait_for=''))
