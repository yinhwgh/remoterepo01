...
class Test(BaseTest):
    ...
    Storage set to LD. ATD>"str"[;]  - MO Voice and Data, Fax Call if supported.
    Subscriber: 2
    '''
    ...
    def run(test):
        ...
        for types in types_mo:
            test.function_check(1,types,'GSM')
        test.log.step('2. Test different type with UCS2 charset')
        for types in types_mo:
            test.function_check(2,types,'UCS2')
    ...
    def function_check(test,i,type_n,charset):
        if type_n == '145':
        ...
        test.log.step(f'{i}.3 Set up call from LD by atd>str;.')
        test.expect(test.dut.dstl_set_pb_memory_storage('LD'))
        test.dut.at1.send_and_verify(f'atd>"{test.convert_char_ucs2(test_text, charset)}";')
        test.expect(test.r1.at1.wait_for('RING'))
        test.expect(test.dut.at1.send_and_verify('at+clcc', f'1,0,3,0,0,"{read_number}",.*'))
        test.sleep(2)
        test.dut.dstl_release_call()
