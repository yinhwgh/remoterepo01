...
class TpAtScfgFastSwitchOff(BaseTest):
    ...
    def run(test):
        test.log.info("\n\t\t  1. check and deregister from network ")
        ...
        test.log.info("\n\t\t  2. test: check command without PIN ")
        ...
        ret = test.check_timing("3. SMSO normal without PIN", 9)
        ...
        ret = test.check_timing("4 SMSO=FAST without PIN", 1)

        test.dut.dstl_turn_on_igt_via_dev_board()
        test.expect(test.dut.at1.wait_for(".*SYSSTART.*", timeout=test.wforsysstarttimer))


        test.log.info("\n\t\t  5. test: check command with PIN ")
        test.sleep(5)
        ...
        ret = test.check_timing("6. SMSO normal with PIN", maxduration=9)
        ...
        test.time1 = time.perf_counter()
        test.expect(test.dut.devboard.wait_for(".*PWRIND: 1.*", timeout=5))
        ret = test.check_timing("7. SMSO FAST with PIN", maxduration=1)

        test.dut.dstl_turn_on_igt_via_dev_board()
        test.expect(test.dut.at1.wait_for(".*SYSSTART.*", timeout=test.wforsysstarttimer))


        test.log.info("\n\t\t  8. test: check at command with invalid paramter ")
        ...
        test.log.info("\n\t\t  9. check fast shutdown by GPIO4 ")
        ...
        test.time1 = time.perf_counter()
        test.expect(test.dut.devboard.wait_for(".*PWRIND: 1.*", timeout=5))
        ret = test.check_timing("10. GPIO-Shutdown without PIN", maxduration=1)
        ...
        test.time1 = time.perf_counter()
        test.expect(test.dut.devboard.wait_for(".*PWRIND: 1.*", timeout=5))
        ret = test.check_timing("11. GPIO-Shutdown without PIN", maxduration=1)
