...
class Test(BaseTest):
    ...
    def run(test):
        ...
        test.log.step('2: APN in cgdcont and APN in AT^SNLWM2M=cfg/ext,MODS - Start')
        ...
        test.log.step("3. Create device on MODS")
        ...
        test.log.step(
            "6. Wait until device is registered - key word 'registered' should occur at the end of the response")
        ...
        test.log.step("7. Find the job and wait until it is finished")
        ...
        test.log.info("Call and check response of AT-commands one by one which AIDON is using")
        test.expect(test.dut.at1.send_and_verify('AT&F', expect='.*OK.*'))
        test.expect(test.dut.at1.send_and_verify('AT\\Q3', expect='.*OK.*'))
        if "\Q3" in test.dut.at1.last_response:
            test.log.info("RTS/CTS flow control is properly set")
        else:
            test.dut.at1.send_and_verify('AT\\Q3', expect='.*OK.*')
        ...
        if "^SIND: ceer,1,99" in test.dut.at1.last_response:
            test.log.info("Turn on URC ^CIEV for extended error report is set")
        else:
            test.dut.at1.send_and_verify('AT^SIND="CEER",1,99', expect='.*OK.*')
        ...
        test.expect(test.dut.at1.send_and_verify('AT+CGREG=1', expect='.*OK.*'))
        test.expect(test.dut.at1.send_and_verify('AT^SXRAT=10,7', expect='.*OK.*'))
        test.expect(test.dut.at1.send_and_verify('AT^SXRAT=7,7', expect='.*OK.*'))

        test.expect(
            test.dut.at1.send_and_verify('AT^SCFG="Radio/Band/CatM","80004","0"', expect='.*OK.*'))
        test.expect(
            test.dut.at1.send_and_verify('AT^SCFG="Radio/Band/CatNB","80004","0"', expect='.*OK.*'))
        test.expect(test.dut.at1.send_and_verify('AT^SSET=1', expect='.*OK.*'))
        test.expect(test.dut.at1.send_and_verify('AT+CMEE=2', expect='.*OK.*'))
        test.expect(test.dut.at1.send_and_verify('AT^SLED=2', expect='.*OK.*'))
        test.expect(test.dut.at1.send_and_verify('AT+CGEREP=2', expect='.*OK.*'))
        test.expect(test.dut.at1.send_and_verify('AT+CEDRXS=0,4', expect='.*OK.*'))
        test.expect(test.dut.at1.send_and_verify('AT+CEDRXS=0,5', expect='.*OK.*'))
        ...
        test.log.step("8. Check if Bootstrap state is deactive and subscription is active")
        ...
        test.log.step(
            "9. Check if APN of subscription occurs in response of 'at+cgdcont?' and 'AT^SNLWM2M=cfg/ext,MODS'")
        test.expect(test.dut.at1.send_and_verify('at+cgdcont?', expect='.*OK.*'))
        last_response = test.dut.at1.last_response
        ...
        test.log.step("11. Call ping if internet connection is established via operational profile")
