...
# location: Berlin
# test case: UNISIM01-284 UNISIM01-285 UNISIM01-286 UNISIM01-287 UNISIM01-288 UNISIM01-289 UNISIM01-290
#            UNISIM01-291 UNISIM01-292 UNISIM01-293 UNISIM01-322 UNISIM01-323 UNISIM01-324 UNISIM01-325
#            UNISIM01-326 UNISIM01-327
# Robustness: switch off the module in after different staates
# UNISIM01-322 - Robustness: power cut in the middle of sequence -> during init after URC "CIEV: simstatus,4"
#   point 1: during init after URC "CIEV: simstatus,4"
# UNISIM01-323 - Robustness: power cut in the middle of sequence -> 5 sec after MODS start
#   point 2: 5 sec after at^srvctl="MODS","start"
# UNISIM01-287 - Robustness: power cut in the middle of sequence -> after "Authenticate server done"
#   point 3: after "Authenticate server done"
# UNISIM01-289 - Robustness: power cut in the middle of sequence -> after "PPR conditions check done"
#   point 4: after "PPR conditions check done"
# UNISIM01-324 - Robustness: power cut in the middle of sequence -> after at^snlwm2m=status/srv,<list>
#   point 5: after at^snlwm2m=status/srv,<list>
# UNISIM01-290 - Robustness: power cut in the middle of sequence -> after "Get Bound Profile Package done"
#   point 6: after "Get Bound Profile Package done"
# UNISIM01-291 - Robustness: power cut in the middle of sequence -> after "Load Bound Profile Package done -
#                Profile download successful"
#   point 7: after "Load Bound Profile Package done - Profile download successful"
# UNISIM01-292 - Robustness: power cut in the middle of sequence -> after "Send PIR notification to server done"
#   point 8: after "Send PIR notification to server done"
# UNISIM01-325 - Robustness: power cut in the middle of sequence -> after "LPA/Profiles/Download",0x0000
#   point 9: after "LPA/Profiles/Download",0x0000
# point 10: after ^SRVACT: "MODS","update","finished"
# UNISIM01-327 -
# point 11: after "CIEV: simstatus,5
# UNISIM01-284 - Robustness: power cut in the middle of sequence -> after "Activation code decoded"
#   point 12: after Activation code decoded
# UNISIM01-285 - Robustness: power cut in the middle of sequence -> after "Initiate authentication done"
#   point 13: after "Initiate authentication done"
# UNISIM01-286 - Robustness: power cut in the middle of sequence -> after "SM-DP+ address checking done"
#   point 14: after ,"SM-DP+ address checking done"
# UNISIM01-288 - Robustness: power cut in the middle of sequence -> after "Authenticate client done"
#   point 15: after ,"Authenticate client done"
# UNISIM01-293 - Robustness: power cut in the middle of sequence -> after "Clear PIR notification done"
...
KPI_TYPE = "bin"
testkey = 'UNISIM01-284 UNISIM01-285 UNISIM01-286 UNISIM01-287 UNISIM01-288 UNISIM01-289 UNISIM01-290 UNISIM01-291 ' \
          'UNISIM01-292 UNISIM01-293 UNISIM01-322 UNISIM01-323 UNISIM01-324 UNISIM01-325 UNISIM01-326 UNISIM01-327'
power_break_at_point = 0
test_abort = False
imei=''


# t1 = test.thread()
# t2 = test.thread(test.power_break, 1, 1.10)
# t1.join()
# t2.join()
#
# if test_abort:  # is set in power_break
# # abort full testcase, no cleanup etc.
#   raise Exception('Power break was done - Test abort')
...
class Test(BaseTest):
    ...
    def power_break(test,current_point, timer=0.0):
        global power_break_at_point
        global test_abort
        global imei
        test.log.info('Power break point at: >' + str(power_break_at_point) + '< current point: >'+ str(current_point) + '< Timer: >' + str(timer) + '<')
        if power_break_at_point == current_point:
            test.log.info('Wait time before power off is: >' + str(timer) + '<')
            test.sleep(timer)
            test.log.info ('Power off the module via McTest')
            result = test.expect(test.dut.dstl_turn_off_vbatt_via_dev_board())
            test.sleep(20)
            result = test.expect(test.dut.dstl_turn_on_vbatt_via_dev_board()) and result
            test.log.info ('check if a subscription exit for these IMEI')
            ...
            test.expect(test.dut.at1.wait_for(".*SYSSTART.*", timeout=test.systart_timeout))
            #test.sleep(5)
            test_abort = True
            overall_result = True
            ...
            test.expect(test.dut.at1.send_and_verify('at^srvctl="MODS","status"', wait_for='OK', timeout=30))
            overall_result = test.expect(test.dut.at1.wait_for(".*SYSSTART AIRPLANE MODE.*", timeout=1200)) and overall_result
            overall_result = test.expect(test.dut.at1.wait_for(".*SYSSTART.*", timeout=600)) and overall_result
            overall_result = test.dut.at1.wait_for('CREG: 5', timeout=600.0, greedy=False) and overall_result
            test.sleep(10)
            ...
            if overall_result:
                test.expect(True)
                test.log.info ('subscription download works fine after power break')
            else:
                test.expect(False)
                test.log.info('subscription download is NOT working after power break')
            test.log.info ('########################################################')
        else:
            result = False
        return(result)
    ...
    def setup(test):
        ...
        if power_break_at_point == 0:
            power_break_at_point = test.power_break_at_point
        if power_break_at_point == 0 or power_break_at_point>20:
            test.expect(False)
        test.log.info ('Run the Test and make the power break at step: >' + str(power_break_at_point) + '<')
        ...
        if power_break_at_point == 1:
            # for Step1 the device must create here
            # for all other cases later
            test.log.step('Step 0.1a: Create/check device on MODS (power break1 extra)- Start')
            test.log.info("This is your IMEI: " + imei)
...
#        test.dut.dstl_detect()
        ...
        t2 = test.thread(test.power_break, 1, 3.10)  # power break point 1
        t1.join()
        t2.join()

        if test_abort:  # is set in power_break
            # abort full testcase, no cleanup etc.
            raise Exception('Power break was done for point ' + str(power_break_at_point) + ' - Test abort')
    ...
    def run(test):
        ...
        if power_break_at_point == 0 or power_break_at_point>20:
            test.expect(False)
        test.log.info ('Run the Test and make the power break at step: >' + str(power_break_at_point) + '<')
        # test.expect(test.power_break(1))
        ...
        if "service is running" in test.dut.at1.last_response:
            ...
            t1 = test.thread(test.dut.at1.send_and_verify_retry, 'at^srvctl="MODS","start"',
                             ...
                             retry_expect="service start failed")
            t2 = test.thread(test.power_break, 2, 5) # power break point 2
            t1.join()
            t2.join()
            if test_abort:  # is set in power_break
                # abort full testcase, no cleanup etc.
...
########
        t1 = test.thread(test.dut.dstl_collect_result, 'Step xx: Send pending notifications again',
                                     test.dut.dstl_send_pending_notifications)
        t2 = test.thread(test.power_break, 12, 25)  # power break point 12
        t1.join()
        t2.join()
        if test_abort:  # is set in power_break
            # abort full testcase, no cleanup etc.
