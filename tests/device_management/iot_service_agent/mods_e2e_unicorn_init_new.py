...
# location: Berlin
# TC: UNISIM01-108: E2E tests needs to be started from a well configured device,
...
class Test(BaseTest):
    ...
    def run(test):
        ...
        if len(profile_list) == 1:
            ...
            test.log.step('Step 7: Check network registration - Start')
            test.dut.dstl_collect_result('Step 7: Check network registration',
            ...
            test.log.step('Step 8: Delete all subscription on module - Start')
            test.dut.dstl_collect_result('Step 8: Delete all subscription on module',
            ...
            test.log.step('Step 10: Create/check device on MODS - Start')
            imei = test.dut.dstl_get_imei()
            test.log.info("This is your IMEI: " + imei)
            result, device_id = test.dut.dstl_create_device_on_mods(imei)
            test.dut.dstl_collect_result('Step 10: Create/check device on MODS', result)

            test.log.step('Step 11: Check registering on MODS - Start')
            test.dut.dstl_collect_result('Step 11: Check registering on MODS',
                                         test.dut.dstl_check_registering_on_mods())

            test.log.step('Step 12: Send pending notifications - Start')
            test.dut.dstl_collect_result('Step 12: Send pending notifications',
            ...
            test.log.step(
                'Step 13: Get all subscriptions IDs which belong to your IMEI and delete all subscriptions - Start')
            iccids = test.dut.dstl_get_all_iccids(imei)
            subscription_ids = test.dut.dstl_get_all_subscription_ids(imei)

            test.log.info(
                "We have decided to use drop function instead of delete to delete all ICCIDs from MODS server")
            result = test.dut.dstl_drop_all_subscriptions(subscription_ids, iccids)
            ...
            test.dut.dstl_collect_result(
                'Step 13: Get all subscriptions IDs which belong to your IMEI and delete all subscriptions',
                result)

            test.log.step('Step 14: Send pending notifications again - Start')
            test.dut.dstl_collect_result('Step 14: Send pending notifications again',
                                         test.dut.dstl_send_pending_notifications())
            ...
            test.log.step('Step 15: Create already deleted subscriptions back on MODS - Start')

            test.log.step('Step 15: Create already deleted subscriptions back on MODS - Start')

            result, test.iccid_results_list = test.dut.dstl_create_all_subscriptions(iccids)
            test.dut.dstl_collect_result(
                'Step 15: Create already deleted subscriptions back on MODS', result)
            ...
            test.log.step('Step 16: Check status of module - Start')
            result = True
            ...
            test.dut.dstl_collect_result('Step 16: Check status of module', result)
