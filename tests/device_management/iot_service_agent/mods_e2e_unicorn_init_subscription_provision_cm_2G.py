...
# location: Berlin
# test case: UNISIM01-253: Use Case: Initial Connectivity Provisioning with Instant Connect over 2G
...
class Test(BaseTest):
    ...
    def setup(test):
        ...
        if "EXS82" not in test.dut.product and "TX82" not in test.dut.product :
            test.expect(False, critical=True, msg="EXS82/TX82 is expected, but not available - test will be aborted")

        if test.rat:   # check if parameter rat is available
            if test.rat is not "2G":
                test.log.info("save original rat:" + test.rat)
                test.rat_original = test.rat
                test.changed_rat = True

        test.log.info("set rat to: 2G")
        test.rat = '2G'  # set rat to 2G
    ...
    def run(test):
        ...
        while job_status != 'succeeded' and wait_count < max_count:
            dstl.log.com("Loop " + str(wait_count) + "/" + str(max_count))
            test.dut.at1.send_and_verify_retry("at^smoni", expect="OK", retry=5, sleep=30,
        ...
        test.dut.dstl_collect_result('Step 16: Get all all attributes from device', result)
