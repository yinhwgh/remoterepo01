...
#        test.dut.dstl_detect()
    ...
    def run(test):
        ...
        test.log.step('Step 16: Read all available profiles - Start')
        _, profiles = test.dut.dstl_get_profile_info()
        log_body(profiles)

        bt_profile = profiles[0]
        bt_iccid = bt_profile.get("iccid")

        op_profile = profiles[1]
        op_iccid = op_profile.get("iccid")

        if len(profiles) == 2:
            test.log.info(
                "Only bootstrap and one operational profile was found, continue your test")
        else:

            test.log.info("Does not make sense to continue")
            test.expect(False, critical=True)
        ...
        test.log.info(f'{40 * "*"} Get all attributes of your Device from IoT Suite {40 * "*"}')
        ...
        test.log.step('Step 17: Check if bootstrap-iccid occurs in device attributes "object 33097" - Start')
        bt_iccid = bt_iccid.replace("\"","")
        test.expect(bt_iccid == device_obj['shadow']['reportedState']['instances']['33097']['0']['0'])

        test.log.step('Step 18: Check if operation-iccid is active and occurs in device attributes "object 33097" - Start')
        op_iccid = op_iccid.replace("\"", "")
        test.expect(op_iccid == device_obj['shadow']['reportedState']['instances']['33097']['1']['0'])
        test.expect(1 == device_obj['shadow']['reportedState']['instances']['33097']['1']['5'])
