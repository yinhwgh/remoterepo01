...
class Test(BaseTest):
    ...
    # 0--power off during register device.
    # 1--power off during download prfole.
    # 2--power off during trigger restart.
    ...
    def run(test):
        test.log.info('*' * 80)
        test.log.step(f'Power off test during connectivity provision.')
        test.log.info('*' * 80)
    ...
    def connectivity_provision_with_different_rat(test):
        ...
        while True:
            ...
            test.log.step(f"Step 3. Power off during connectivity provision and continue.")
            test.expect(abnormal_condition_during_connectivity_activation(test, step=3, condition='power_off'))
            test.log.step("Step 4.Fall back to bootstrap enabled and repeat step 1-3")
