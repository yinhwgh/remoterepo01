...
class Test(BaseTest):
            ...
            if test.mctest_present:
                ...
                if 'MC-Test4' in test.dut.dstl_get_dev_board_version():
                    ...
                    if 'blnlnx5.gemalto' in label:
                        test.log.com(
                            '***** workaround: problems with CI - do not perform power consumption measurement - computer name: ' + label)
                    else:
                        test.log.com(
                            '***** "MC-Test4" found - power consumption measurement will be started later *****')
    ...
    def run(test):
        ...
        if result == True:
            ...
            if test.mc_test4_available:
                test.log.info(
                    f'Stop and save KPI NW scan power consumption: {test.kpi_name_registration_power_consumption}')
                ...
                if power_value and result:
                    dstl.test.kpi_store(name=test.kpi_name_registration_power_consumption, value=power_value[0],
                                        type='num', device=test.dut)
        else:
            if test.mc_test4_available:
                test.log.info(
                    'McTest4 available, but no network registration - stop power consumption measurement => NO KPI')
                test.dut_devboard.send_and_verify("mc:ccmeter=off", expect=".*OK.*")
                test.dut_devboard.send_and_verify("mc:ccmeter=read", expect=".*OK.*")
        ...
        test.dut.dstl_collect_result('Step 13: Send pending notifications again',
        ...
        test.dut.dstl_collect_result('Step 15: Check IP address and APN in <list> command',
        ...
        test.log.step('Step 20: Check if network scan is finisehd successfully, value should be 2 - Start')
        try:
            dev_33096_0_32 = dev_obj['shadow']['reportedState']['instances']['33096']['0']['32']
        except:
            test.log.warn('value of object 33096/0/32 could not be found')
            test.dut.dstl_collect_result('Step 20: value of object 33096/0/32 could not be found, value should be 2', False)
        else:
            if dev_33096_0_32 == 2:
                result = True
            else:
                result = False
                test.log.warn('value of 33096/0/32 should be 2, but is: ' + str(dev_33096_0_32))
            test.dut.dstl_collect_result('Step 20: Check if network scan is finished successfully, value should be 2 (33096/0/32)', result)



        test.log.step('Step 21: Check output of network scan, cell_id should occur in REST Api response - Start')
        try:
            dev_33096_0_33_3 = dev_obj['shadow']['reportedState']['instances']['33096']['0']['33']['3']
        except:
            test.log.warn('Value of object 33096/0/33/3 could not be found')
            test.dut.dstl_collect_result('Step 21: Value of object 33096/0/33/3 could not be found', False)
        else:
            if "cell_id" in dev_33096_0_33_3:
                result = True
            else:
                result = False
                test.log.warn('"cell_id" is not in dev_33096_0_33_3: ' + str(dev_33096_0_33_3))
            test.dut.dstl_collect_result('Step 21: Check output of network scan, "cell_id" should occur in REST Api response (33096/0/33/3)',
                             result)
            # test.dut.dstl_collect_result('Step 21: Check output of network scan, cell_id should occur in REST Api response',
            #                      test.expect("cell_id" in
            #                                  dev_obj['shadow']['reportedState']['instances']['33096']['0']['33'][
            #                                      '3']))
