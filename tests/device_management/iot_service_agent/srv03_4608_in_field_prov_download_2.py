...
# location: Berlin
# test case: SRV03-4608 - In-Field Provisioning : enable "bootstrap / provisioning" profile when AT command to set downloadMode = 2
...
class Test(BaseTest):
    """
    In-Field Provisioning : enable "bootstrap / provisioning" profile when AT command to set downloadMode = 2
    """
    systart_timeout = 300
    ...
    def setup(test):
        ...
        test.log.step(
            "Step 1 at^susmc? throws per default only 2 parameters. To make missing parameter visible call following top secret command")
    ...
    def run(test):
        ...
        test.dut.dstl_set_prid_wo_agent(prid_name = "UNICORN-POC")

        test.log.step("Step 2. Set Download Mode to 2")
        test.dut.dstl_set_download_mode(download_mode = 2)
        ...
        test.log.step(
        "Step 4. Wait until device is registered - key word 'registered' should occur at the end of the response")
        ...
        test.log.step("Step 5. Find the job and wait until it is finished")
    ...
    #        test.dut.at1.wait_for("^SYSSTART", timeout=test.systart_timeout)
        ...
        test.log.step('Step 6: Check device on MODS has group assigned (' + test.project_name + ')- Start')
        ...
        test.dut.dstl_collect_result(
            'Step 6: Check device on MODS has group assigned (of project: >' + test.project_name + '<)',
            group_id_in_project)
        ...
        test.log.step("Step 7. Check via at^susma? if Bootstrap profile is deactive and operational profile is active")
        ...
        test.log.step("Step 8. Check via at^susmc? if Bootstrap profile is deactive and operational profile is active")
        ...
        test.log.step('Step 14: Get all attributes from device - Start')
