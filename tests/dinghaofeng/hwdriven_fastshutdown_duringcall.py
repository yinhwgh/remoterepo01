...
class Test(BaseTest):
    """
       TC0108214.001 - HWDrivenFastShutdownDuringCall
    """
...
def main_process(test):
    for i in range(1):
        ...
        test.dut.devboard.send_and_verify('atd10011;', 'OK', wait_for=".*OK.*")
        test.sleep(random.randint(0, 10))
        test.dut.devboard.send_and_verify('ath', 'OK', wait_for=".*OK.*")
