...
class Test(BaseTest):
    ...
    def run(test):
        ...
        test.expect(test.dut.at1.send_and_verify(" AT^SIND=nitz,1", '.*\^SIND: nitz,1,\"\",\+00.*'))
        test.sleep(5)
        # enter pin with at+cpin="PIN1" and wait for registration
        test.dut.dstl_enter_pin()
        #test.sleep(5)
        # wait for arriving indicators +ciev:  eons
        test.expect(test.dut.at1.wait_for("\+CIEV: nitz,\"", timeout=20))
        ...
        test.sleep(10)
        test.expect(test.dut.at1.send_and_verify(" AT+cops=0", "\+CIEV: nitz,\"", timeout=20))
        ...
        test.dut.at1.send_and_verify(" AT^SIND=nitz,0", ".*\^SIND: nitz,0,.*")
