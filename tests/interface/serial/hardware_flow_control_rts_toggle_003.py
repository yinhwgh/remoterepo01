...
def execute_step_2(test):
    ...
    for sms_position in range(1, test.ITERATIONS):
...
def execute_step_3(test):
    ...
    test.expect(len(result) == test.ITERATIONS-1)
...
def fill_sms_memory(test):
    ...
    current_sms_account = int(test.dut.dstl_get_sms_count_from_memory()[memory_index])
    for message_index in range(memory_capacity-current_sms_account):
