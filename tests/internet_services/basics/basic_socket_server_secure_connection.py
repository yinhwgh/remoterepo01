...
class Test(BaseTest):
    ...
    def run(test):
        test.log.h2("Executing script for test case: 'TC0093957.001 BasicSocketServerSecureConnection'")
        ...
        test.expect(check_and_delete_certificates(test))
        ...
        test.expect(test.socket_dtls.dstl_get_service().dstl_open_service_profile())
        test.expect(test.socket_dtls.dstl_get_urc().dstl_is_sis_urc_appeared("0", "77", '"The certificate does not exist"'))

        test.log.step("28. Delete all certificates")
        check_and_delete_certificates(test)
...
def check_and_delete_certificates(test):
    amount_of_certificates = test.certificates.dstl_count_uploaded_certificates()
    if amount_of_certificates == 0:
        return True
    for index in range(amount_of_certificates-1, -1, -1):
        test.certificates.dstl_delete_certificate(index)
    return test.certificates.dstl_count_uploaded_certificates() == 0
