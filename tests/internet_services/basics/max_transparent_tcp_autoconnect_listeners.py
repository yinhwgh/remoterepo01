...
class Test(BaseTest):
    """TC0103289.001, TC0103289.002 Check the behaviour of module during transparent connection and maximum number of
    opened listeners with autoconnect" parameter."""
    ...
    def run(test):
        ...
        test.log.step("2) Set service profiles: max amount of TCP transparent listeners on DUT with enabled "
                      "autoconnect option.")
        test.socket_dut = []
        etx = 26
        for profile_number in range(10):
            current_port = "888" + str(profile_number)
            test.socket_dut.append(SocketProfile(test.dut, profile_number, conn_setup_dut.dstl_get_used_cid(),
                                                 protocol="tcp", host="listener", localport=current_port, autoconnect=1,
                                                 etx_char=etx))
        ...
        dut_ip = test.socket_dut[0].dstl_get_parser().dstl_get_service_local_address_and_port(ip_version='IPv4').\
            split(":")
        test.socket_rem = []
        for profile_number in range(10):
            current_port = "888" + str(profile_number)
            test.socket_rem.append(SocketProfile(test.r1, profile_number, conn_setup_dut.dstl_get_used_cid(),
                                                 protocol="tcp", host=dut_ip[0], port=current_port, etx_char=etx,
        ...
        for profile_number in range(10):
            ...
            test.expect(test.socket_rem[profile_number].dstl_get_service().dstl_open_service_profile())
            test.expect(test.socket_rem[profile_number].dstl_get_urc().dstl_is_sisw_urc_appeared('1'))

            test.log.step("6) Send 10 x 100 bytes from DUT to Remote and from Remote to DUT (in transparent mode)")
            ...
            test.expect(test.socket_rem[profile_number].dstl_get_service().dstl_enter_transparent_mode())
            ...
            test.expect(test.socket_dut[profile_number].dstl_get_parser().dstl_get_service_state() ==
                        ServiceState.CONNECTED.value)
            test.expect(test.socket_dut[profile_number].dstl_get_parser().dstl_get_service_data_counter("rx") == 1000)
            test.expect(test.socket_dut[profile_number].dstl_get_parser().dstl_get_service_data_counter("tx") == 1000)

            test.expect(test.socket_rem[profile_number].dstl_get_parser().dstl_get_service_state() ==
                        ServiceState.UP.value)
            test.expect(test.socket_rem[profile_number].dstl_get_parser().dstl_get_service_data_counter("rx") == 1000)
            test.expect(test.socket_rem[profile_number].dstl_get_parser().dstl_get_service_data_counter("tx") == 1000)

            test.log.step("9) Repeat steps 5-9 for other client profiles times")

    def cleanup(test):
        test.log.step("10) Close all services on DUT and Remote")
        try:
            for profile_number in range(10):
                test.expect(test.socket_dut[profile_number].dstl_get_service().dstl_close_service_profile())
                test.expect(test.socket_rem[profile_number].dstl_get_service().dstl_close_service_profile())
