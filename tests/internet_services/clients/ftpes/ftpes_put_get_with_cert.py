...
class Test(BaseTest):
    ...
    Correct communication starts out as plain FTP (over e.g. port 21), but is upgraded to
    TLS/SSL encryption.
    ...
    def run(test):
        ...
        ftpes_client_put_get = FtpProfile(test.dut, 0, used_cid, command="put", alphabet=1,
                                          ftpes=True, files=test.file_name, secopt="1",
                                          secure_connection=True)
        test.ftpes_server = FtpesServer("IPv4", test, used_cid)
        ...
        test.log.step("5. Open defined profile. Check in URCs if encryption is used while "
                      "connection. ")
        test.expect(ftpes_client_put_get.dstl_get_service().dstl_open_service_profile())
        test.expect(ftpes_client_put_get.dstl_get_urc().dstl_is_sis_urc_appeared(urc_cause="0",
                                                    urc_info_id="200",
        ...
        test.expect(ftpes_client_put_get.dstl_get_parser().dstl_get_service_state() ==
                    ServiceState.UP.value,
                    msg="Wrong service state")
        test.expect(ftpes_client_put_get.dstl_get_parser().dstl_get_socket_state() ==
                    SocketState.CLIENT.value,
                    msg="Wrong socket state")

        test.log.step("7. Upload file to FTP server (size 1KB) ")
        test.expect(ftpes_client_put_get.dstl_get_service().dstl_send_sisw_command_and_data(
            amount_of_data, eod_flag="1"))
        ...
        test.log.step("10) Define FTPES \"get\" service profile to FTP server (same file that was "
                      "uploaded in step 7).")
        ftpes_client_put_get.dstl_set_ftp_command("get")
        ftpes_client_put_get.dstl_generate_address()
        test.expect(ftpes_client_put_get.dstl_get_service().dstl_load_profile())

        test.log.step("11) Open defined profile. Check in URCs if encryption is used "
                      "while connection.")
        test.expect(ftpes_client_put_get.dstl_get_service().dstl_open_service_profile())
        test.expect(ftpes_client_put_get.dstl_get_urc().dstl_is_sis_urc_appeared(urc_cause="0",
                                                    urc_info_id="200",
        ...
        test.log.step("13) Download whole file from FTP server (wait for proper URC after "
                      "downloading file).")
