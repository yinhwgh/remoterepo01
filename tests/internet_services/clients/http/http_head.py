...
class Test(BaseTest):
    ...
    def run(test):
        test.log.info("Executing script for test case: 'TC0093322.001 HttpHead'")
        ...
        test.log.step("3. Define HTTP HEAD profile to HTTP web page.")
        srv_id = "0"
        con_id = connection_setup.dstl_get_used_cid()
        http_client = HttpProfile(test.dut, srv_id, con_id, http_command="head", host=server_ip_address, port=server_port)
        ...
        test.expect('^SISS: {},"address","http://{}:{}"'.format(srv_id, server_ip_address,
                                                                server_port) in test.dut.at1.last_response)
        test.expect('^SISS: {},"cmd","head"'.format(srv_id) in test.dut.at1.last_response)
        ...
        test.expect(http_client.dstl_get_urc().dstl_is_sis_urc_appeared("0", "2200", '"Http connect {}:{}"'.format(server_ip_address, server_port)))
        ...
        read_data = http_client.dstl_get_service().dstl_read_return_data(1500)
        test.expect(http_client.dstl_get_urc().dstl_is_sisr_urc_appeared("2"))

        test.log.step("8. Check received message.")
        test.expect('Server:' in read_data)
        test.expect('Date:' in read_data)
        test.expect('Connection:' in read_data)
        ...
        test.log.step("10. Close HTTP HEAD service.")
