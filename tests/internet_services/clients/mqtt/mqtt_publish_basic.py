...
class Test(BaseTest):
    """
    Intention:
    To check correct Module behavior while opening MQTT publish profile.
    ...
    2. Define Mqtt Publish profile with mandatory set of parameters using SISS command
    ...
    def run(test):
        test.log.info("TC0107976.001 MqttPublish_basic")
        ...
        test.log.step('2. Define Mqtt Publish profile with mandatory set of parameters using'
                      ' SISS command')
        test.expect(test.mqtt_server.dstl_run_mqtt_server())
        test.mqtt_publish = MqttProfile(test.dut, "0", test.connection_setup.dstl_get_used_cid(),
                                        alphabet=1, cmd="publish", topic=test.topic,
                                        client_id=test.client_id)
        test.mqtt_publish.dstl_set_parameters_from_ip_server(test.mqtt_server)
        test.mqtt_publish.dstl_generate_address()
        test.expect(test.mqtt_publish.dstl_get_service().dstl_load_profile())

        test.log.step('3) Open Mqtt profile using SISO command')
        test.expect(test.mqtt_publish.dstl_get_service().dstl_open_service_profile())
        ...
        test.expect(test.mqtt_publish.dstl_get_service().dstl_close_service_profile())
        test.mqtt_server.dstl_stop_mqtt_server()

    def cleanup(test):
        test.expect(test.mqtt_publish.dstl_get_service().dstl_close_service_profile())
        test.expect(test.mqtt_publish.dstl_get_service().dstl_reset_service_profile())
        try:
            test.mqtt_server.dstl_stop_mqtt_server()
