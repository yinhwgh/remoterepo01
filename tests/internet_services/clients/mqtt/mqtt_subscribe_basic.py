...
class Test(BaseTest):
    """
    Intention:
    To check correct Module behavior while opening MQTT subscribe profile.
    ...
    2. Define Mqtt Subscribe profile with mandatory set of parameters using SISS command
    ...
    def run(test):
        test.log.info("TC0107974.001 MqttSubscribe_basic")
    ...
    def cleanup(test):
        test.expect(test.mqtt_subscribe.dstl_get_service().dstl_close_service_profile())
        test.expect(test.mqtt_subscribe.dstl_get_service().dstl_reset_service_profile())
        try:
            test.mqtt_server.dstl_stop_mqtt_server()
