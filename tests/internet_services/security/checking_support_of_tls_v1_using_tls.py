...
class Test(BaseTest):
    ...
    def run(test):
        ...
        test.log.info("TC0102301.002 - CheckingSupportOfTLSv1UsingTLS")

        test.log.step("1) Run OpenSSL server using TLSv1")
        test.ssl_server = SslServer("IPv4", "socket_tls", "TLS_RSA_WITH_AES_256_CBC_SHA")
        ...
        test.sleep(10)
        test.expect(test.socket_service.dstl_get_urc().dstl_is_sis_urc_appeared(urc_info_id="62"))
        ...
        test.sleep(30)
        test.expect("wrong version number" in test.ssh_server.last_response,
                    msg="Incorrect or missing \"wrong version number\" in server response!")
