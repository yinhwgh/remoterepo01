...
class Test(BaseTest):
    ...
    def run(test):
        ...
        test.log.info("TC0102312.001 - FTPES_PutGet_WithoutCert")
        ...
        test.ftpes_put = FtpProfile(test.dut, 0, connection_setup_object.dstl_get_used_cid(), command="put",
                                      files="test.txt", secopt="0", secure_connection=True, ftpes=True)
        test.ftpes_server = FtpServer("IPv4", test, connection_setup_object.dstl_get_used_cid())
