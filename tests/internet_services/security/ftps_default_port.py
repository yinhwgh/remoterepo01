...
class Test(BaseTest):
    ...
    def run(test):
        ...
        response = test.dut.at1.last_response.split("\n")
        test.expect(response[3].count(":") == 2, msg="Correct address parameter in module")
        ...
        rem_address = ftps_client.dstl_get_parser().dstl_get_service_remote_address_and_port("IPv4")
        test.expect(rem_address.contains(test.ftps_server.dstl_get_server_port()))
