...
class Test(BaseTest):
    ...
    def run(test):
        test.log.h2("Executing script for test case: 'TC0093572.001/002 InternetProfileStorageBasicUsb'")
        ...
        for usb_interface in usb_interfaces:
            ...
            test.log.step("15. Repeat the scenario above on each USB interface (if module support more than one).")
...
def define_service_profile(test, profile_number, service_type):
    if 'http' in service_type:
        service_profile = HttpProfile(test.dut, profile_number, '1', http_command="head", host="www.httpbin.org")
        ...
        service_profile = SocketProfile(test.dut, profile_number, '1', protocol="udp", host="122.242.234.89", port="64321")
    elif 'socktcp' in service_type:
        service_profile = SocketProfile(test.dut, profile_number, '1', protocol="tcp", host="192.222.134.224", port="61234")
