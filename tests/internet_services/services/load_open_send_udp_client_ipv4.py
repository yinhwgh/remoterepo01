...
class Test(BaseTest):
    ...
    def run(test):
        ...
        test.log.info("Executing script for test case: 'LoadOpenSendUdpClient_IPv4'")

        dstl_set_scfg_tcp_with_urcs(test.dut, "on")

        test.echo_server = EchoServer('IPv4', "UDP", test_duration=8)
        connection_setup_object = dstl_get_connection_setup_object(test.dut, ip_version='IPv4')

        test.log.step("1) Set IPv4 UDP client socket profile on module")
        ...
        test.expect(counter_of_failed_iterations < iterations*0.2, msg="Amount of lost packets is greater than 20%.")
