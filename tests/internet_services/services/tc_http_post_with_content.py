...
class Test(BaseTest):
    ...
    def run(test):
        ...
        test.http_profile = HttpProfile(test.dut, srv_id, con_id, address=http_post_server, http_command="post")
        ...
        for hc_content_value in hc_content_values:
            ...
            if hc_content_value == 1:
                ...
                test.log.step("B) Repeat points 4-11 with hcContent set to maximum data length + 10 bytes.")
            ...
            if hc_content_value == 265:
                test.expect(test.dut.at1.send_and_verify('AT^SISS={},hccontent,"{}"'.format(srv_id, hc_content), expect='.*ERROR.*'))
                break
            ...
            test.expect(test.http_profile.dstl_get_service().dstl_read_data(1500))
            test.expect(test.http_profile.dstl_get_urc().dstl_is_sisr_urc_appeared(2))

            test.log.step("9. Compare transmitted/received data.")
            test.expect('"Content-Length": "{}"'.format(len(hc_content)) in test.dut.at1.last_response)
            test.expect('"data": "{}"'.format(hc_content) in test.dut.at1.last_response)
            test.expect(test.http_profile.dstl_get_parser().dstl_get_service_data_counter("tx") == len(hc_content))
