...
class Test(BaseTest):
    ...
    def run(test):
        test.log.h2("Executing test script for: TC0096180.001 NonTranspTcpClientSendEodWithoutReadData")
        ...
        test.log.step("2. Define and open TCP non-transparent services: listener on remote and client on DUT.")
        ...
        socket_server = SocketProfile(test.r1, socket_listener.dstl_get_urc().dstl_get_sis_urc_info_id(), connection_setup_r1.dstl_get_used_cid())
        ...
        test.log.step("3. Server: Send data 1500x4 bytes.")
        test.expect(socket_server.dstl_get_service().dstl_send_sisw_command_and_data(1500, repetitions=4))

        test.log.step("4. Client: Wait for read URC and check number of received bytes (peek value).")
        test.expect(socket_client.dstl_get_urc().dstl_is_sisr_urc_appeared("1"))
        test.sleep(3)
        test.expect(socket_client.dstl_get_parser().dstl_get_peek_value_of_data_to_read() == 4*1500)

        test.log.step("5. Client: Send data 1500x5 to server without reading.")
        test.expect(socket_client.dstl_get_service().dstl_send_sisw_command_and_data(1500, repetitions=5))

        test.log.step("6. Client: Send EOD flag.")
        ...
        test.expect(socket_client.dstl_get_parser().dstl_get_service_data_counter("TX") == 1500*6)

        test.log.step("8. Read rest of data from receive buffer on both sides.")
        test.expect(socket_server.dstl_get_service().dstl_read_data(1500, repetitions=6))
        test.expect(socket_client.dstl_get_service().dstl_read_data(1500, repetitions=4))

        test.log.step("9. Client: Try to send data from client.")
        test.expect(socket_client.dstl_get_service().dstl_send_sisw_command(1500, expected="ERROR"))

        test.log.step("10. Check state and RX, TX counter.")
        ...
        test.log.step("11. Close connection and delete used profiles.")
