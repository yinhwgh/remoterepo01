...
class Test(BaseTest):
    ...
    def run(test):
        ...
        for variable in variables:
            ...
            test.log.step(f"2. Set up internet services for TCP Transparent Socket Client on DUT and TCP Transparent Socket Listener on Remote.\
            Explicitly set up the service to use IPv4. Set Nagle timer to [T = {t}]. [E = {etx}] ")
            ...
            test.socket_dut = SocketProfile(test.dut, 0, test.connection_setup_dut.dstl_get_used_cid(), protocol="tcp",
                                            host=server_ip[0], port=server_ip[1], nagle_timer=t, etx_char=etx)
