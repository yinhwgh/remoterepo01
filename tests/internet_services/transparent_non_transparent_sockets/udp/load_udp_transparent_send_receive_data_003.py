...
class Test(BaseTest):
    ...
    def run(test):
        ...
        for iteration in range (1, iterations + 1):
            ...
            test.expect(test.client.dstl_get_service().dstl_send_data(dstl_generate_data(453), expected="", repetitions=4))
            test.expect(test.endpoint.dstl_get_urc().dstl_is_sisr_urc_appeared("1"))
            test.expect(test.endpoint.dstl_get_service().dstl_read_data(453))
            dut_ip_address_and_port = test.endpoint.dstl_get_service().dstl_get_udp_rem_client()
            test.expect(test.endpoint.dstl_get_service().dstl_read_data(453, repetitions=3))

            test.log.step("6. Send data 4096 bytes to client and wait 30 sec.\n"
                          "Iteration: {} of {}".format(iteration, iterations))
            test.expect(test.endpoint.dstl_get_service().dstl_send_sisw_command_and_data_UDP_endpoint(1024,
                                    dut_ip_address_and_port, eod_flag="0", expected='.*OK.*|.*ERROR.*', repetitions=4))
            test.sleep(30)
            ...
            # one package can be lost for UDP
            test.expect(test.client.dstl_get_parser().dstl_get_service_data_counter("RX") >= 1024*3)
