...
from dstl.call.switch_to_command_mode import dstl_switch_to_command_mode_by_pluses, \
    dstl_switch_to_command_mode_by_dtr, dstl_switch_to_command_mode_by_etxchar
from dstl.auxiliary.generate_data import dstl_generate_data
from dstl.auxiliary.devboard.devboard import dstl_switch_off_at_echo


class Test(BaseTest):
    """Testing operation of specific characters and data line transitions on
    UDP Transparent Socket Service."""
    ...
    def run(test):
        test.log.info("Executing script for: 'TC0093526.001 SocketUdpTransparentToCommandMode'")
        ...
        test.socket = SocketProfile(test.dut, "1", test.connection_setup.dstl_get_used_cid(),
        ...
        test.log.step("2. Enter transparent mode, then send 5x100 bytes of data "
                      "and receive echoed bytes.")
        ...
        test.amount_of_sent_data = len(data)*5
        ...
        test.dut.at1.wait_for(test_sequence, silent=True)
        test.sleep(5)
        test.expect('OK' not in test.dut.at1.last_response)
        test.amount_of_sent_data += len(test_sequence)

        test.log.step("4. Send and receive DLE character to the echo server (DLE + DLE).")
        test.expect(test.socket.dstl_get_service().dstl_send_data(bytes(chr(16), encoding='ascii'),
                                                                  expected=""))
        test.expect(test.socket.dstl_get_service().dstl_send_data(bytes(chr(16), encoding='ascii'),
                                                                  expected=""))
        test.sleep(5)
        test.expect('OK' not in test.dut.at1.last_response)
        test.amount_of_sent_data += 1

        test.log.step("5. Escape transparent mode using \"+++\" sequence. "
        ...
        test.check_data_counters()

        test.log.step("6. Repeat step 2 and escape transparent mode using ETX character "
                      "(DLE + ETX). Check amount of sent data.")
        test.expect(test.socket.dstl_get_service().dstl_enter_transparent_mode())
        data = dstl_generate_data(100)
        test.expect(test.socket.dstl_get_service().dstl_send_data(data, expected="", repetitions=5))
        test.amount_of_sent_data += len(data)*5
        ...
        test.log.step("7. Repeat step 2 and escape transparent mode using DTR toggle method. "
                      "Check amount of sent data.")
        test.expect(test.socket.dstl_get_service().dstl_enter_transparent_mode())
        data = dstl_generate_data(100)
        test.expect(test.socket.dstl_get_service().dstl_send_data(data, expected="", repetitions=5))
