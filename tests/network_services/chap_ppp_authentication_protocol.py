...
class Test(BaseTest):
    """TC0081873.002    CHAP_PPPAuthenticationProtocol

    The module should be able to set up GPRS session when CHAP.

    1.Set type of authentication:
        at^sgauth=1,2,"user","passwd"
    2. Define PDP context:
        at+cgdcont=1,"IPv4v6","your provider APN"
    ...
    def run(test):
        ...
        test.log.step("1.Set type of authentication: at^sgauth=1,2,\"user\",\"passwd\"")
        test.expect(test.dut.at1.send_and_verify("AT^SGAUTH=1,2,\"user\",\"passwd\"", ".*OK.*"))
        
        test.log.step("2. Define PDP context: at+cgdcont=1,\"IPv4v6\",\"your provider APN\"")
