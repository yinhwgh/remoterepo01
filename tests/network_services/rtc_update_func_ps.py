...
class Test(BaseTest):
    """
    Check if internal RTC is updated after NITZ indicator coming. This version of TC is prepared for PS network.
    ...
    --- check if RTC update is possible when NITZ came by registering to the PS network: ---

    11. Obtain NITZ from network again by registering to PS network AT+CGATT=1
    12. Wait for URCs from PS network: "+CGREG: 1", +CTZU:, "+CIEV: nitz"
    13. Query if RTC has been updated to current UTC time: AT+CCLK?
    14. set AT+CCLK far in the past again

    --- check if RTC update is possible when NITZ came by re-registering to the PS network: ---

    15. Obtain NITZ from network again by re-registering to PS network AT+CGATT=0;CGATT=1
    16. Wait for URCs from PS network: "+CGREG: 1", "+CTZU:", "+CIEV: nitz"
    17. Query if RTC has been updated to current UTC time: AT+CCLK?
    """
    ...
    def run(test):
        ...
        test.expect(test.dut.at1.send_and_verify('AT+CGREG=2', '.*OK.*'))
        ...
        test.log.step("11. Obtain NITZ from network again by registering to PS network AT+CGATT=1")
        test.expect(test.dut.at1.send_and_verify('AT+CGATT=1', '.*OK.*'))

        test.log.step("12. Wait for URCs from PS network: '+CGREG: 1', '+CTZU:', '+CIEV: nitz'")
        test.sleep(120)
        buffer = test.dut.at1.last_response + test.dut.at1.read()
        test.expect("+CGREG: 1" in buffer and "+CTZU:" in buffer and "+CIEV: nitz" in buffer)
        ...
        test.log.step("15. Obtain NITZ from network again by re-registering to PS network AT+CGATT=0;CGATT=1")
        test.expect(test.dut.at1.send_and_verify('AT+CGATT=0', '.*OK.*'))
        test.expect(test.dut.at1.send_and_verify('AT+CGATT=1', '.*OK.*'))

        test.log.step("16. Wait for URCs from PS network: '+CGREG: 1', '+CTZU:', '+CIEV: nitz'")
        test.sleep(120)
        buffer = test.dut.at1.last_response + test.dut.at1.read()
        test.expect("+CGREG: 1" in buffer and "+CTZU:" in buffer and "+CIEV: nitz" in buffer)

        test.log.step("17. Query if RTC has been updated to current UTC time: AT+CCLK?")
        test.expect(test.dut.at1.send_and_verify("AT+CCLK?", ".*OK.*"))
        test.expect(datetime.datetime.now().strftime("%y/%m/%d") in test.dut.at1.last_response)
