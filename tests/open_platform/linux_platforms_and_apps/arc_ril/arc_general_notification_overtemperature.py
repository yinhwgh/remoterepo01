...
# location: Wroclaw
# TC0103811.001 arc_general_notification_overtemperature.py
...
class ArcGeneralNotificationOvertemperatur (BaseTest):
    ...
    def run(test):
        ...
        test.dut.dstl_embedded_linux_run_application("LinuxArcRilNotifyShutdown",
                                                     params=["s=2"],
                                                     expect='.*Warining: Overtemperature.*',
                                                     expect_exit_code=0)
        test.sleep(150)
        # run command two times, because the first check runs failes, response was not cached
