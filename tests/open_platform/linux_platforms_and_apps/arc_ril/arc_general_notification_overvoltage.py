...
# location: Wroclaw
# TC0103825.001 arc_general_notification_overvoltage.py
...
class ArcGeneralNotificationOvervoltage (BaseTest):
    ...
    def run(test):
        ...
        test.expect(test.dut.devboard.send_and_verify("MC:VBATT=4400mV", ".*OK.*"))
        test.dut.dstl_embedded_linux_run_application("LinuxArcRilNotifyShutdown",
                                                     params=["s=4"],
                                                     expect='.*Warning: Overvoltage.*',
                                                     expect_exit_code=0)
        test.sleep(150)
