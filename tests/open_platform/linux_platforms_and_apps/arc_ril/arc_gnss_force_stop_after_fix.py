...
# location: Wroclaw
# TC0102208.001 arc_gnss_force_stop_after_fix.py
...
class ArcGnssForceStopAfterFix (BaseTest):
    ...
    def setup(test):
        ...
        test.dut.dstl_embedded_linux_prepare_application("arc_ril\\LinuxArcRilGnssForceStopAfterFix")

    def run(test):

        test.dut.dstl_embedded_linux_run_application("LinuxArcRilGnssForceStopAfterFix",
                expect='(?s).*(?s).*EXP: Test result: PASS(?s).*(?s).*',
                expect_exit_code=0)
