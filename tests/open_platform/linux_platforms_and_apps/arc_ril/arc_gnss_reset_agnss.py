...
# location: Wroclaw
# TC0103761.001 arc_gnss_reset_agnss.py
...
class ArcGnssResetAgnss(BaseTest):
    ...
    def setup(test):
        ...
        test.dut.dstl_embedded_linux_prepare_application("arc_ril\\LinuxArcRilGnssResetAgnss")

    def run(test):

        test.dut.dstl_embedded_linux_run_application("LinuxArcRilGnssResetAgnss",
                expect='(?s).*EXP: PASS(?s).*',
                expect_exit_code=0)
