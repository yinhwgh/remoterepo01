...
# location: Wroclaw
# TC0102218.001 arc_gnss_reset_agnss_while_gnss_enigne_enabled.py
...
class ArcGnssResetAgnssWhileGnssEnigneEnabled (BaseTest):
    ...
    def setup(test):
        ...
        test.dut.dstl_embedded_linux_prepare_application("arc_ril\\LinuxArcRilGnssResetAgnssWhileGnssEngineEnabled")

    def run(test):

        test.dut.dstl_embedded_linux_run_application("LinuxArcRilGnssResetAgnssWhileGnssEngineEnabled",
                expect='(?s).*EXP: error - cmdTemporaryNotAllowed(?s).*',
                expect_exit_code=15)
