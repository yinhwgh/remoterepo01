...
from dstl.auxiliary.devboard.devboard import dstl_switch_off_at_echo, dstl_switch_on_at_echo, dstl_turn_module_usb_off_via_dev_board
...
class Test(BaseTest):
    ...
    def setup(test):
        ...
        result_text += "\t{}:\t{:.2f} mA  (over {} values)\n".format("1", float_average, num_values)
        ...
        test.expect(test.dut.at1.send_and_verify(
            "at^scfg=meopmode/pwrsave,disabled,{},{}".format(test.pwr_save_periode, test.pwr_save_awake)))
    ...
    def cleanup(test):
        ...
        test.expect(test.dut.at1.send_and_verify("at^scfg=meopmode/pwrsave,disabled,52,50"))
