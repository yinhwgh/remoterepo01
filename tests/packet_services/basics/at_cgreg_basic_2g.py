...
class Test(BaseTest):
    ...
    2. Check test read and write commands with PIN (module attach to 2G)
    ...
    def run(test):
        ...
        test.log.step("2. Check test read and write commands with PIN (module attach to 2G)")
        test.expect(dstl_register_to_gsm(test.dut))
        test.sleep(5)
        if test.dut.project.upper() == "SERVAL":
            test.expect(test.dut.at1.send_and_verify("AT+CGREG=?", r".+CGREG: \(0-2,4\).*OK."))
        else:
            test.expect(test.dut.at1.send_and_verify("AT+CGREG=?", r".+CGREG: \(0-2\).*OK."))
        test.expect(test.dut.at1.send_and_verify("AT+CGREG?", r".*\+CGREG: 0,[0|1|2|4].*OK.*"))
        ...
        test.log.step("4. Check functionality for GPRS attach and detach and for de-/re-registration of the module "
                      "(module attach to 2G)")
        if test.dut.project.upper() == "SERVAL":
            for param_n in ["0", "1", "2", "4"]:
                test.log.step("Scenario for CGREG={}".format(param_n))
                if param_n == "4":
                    test.log.info("Currently for 2G network PSM is not configured")
                ...
                test.expect(test.dut.at1.send_and_verify("AT+CGREG?", r".*\+CGREG: {},0.*OK.*".format(param_n)))
                test.expect(dstl_register_to_gsm(test.dut))
                test.expect(test.dut.at1.send_and_verify("AT^SMONI", r".*\^SMONI: 2G,.*OK.*"))
                test.expect(test.dut.at1.send_and_verify("AT+CGREG?", r".*\+CGREG: {},0.*OK.*".format(param_n)))
    ...
    def search_urc(test, param_n, stat):
        if param_n == "0":
            ...
            test.expect(re.search(r".*\+CGREG: {},\".*\",\".*\",(0|3).*".format(stat), test.dut.at1.last_response))
        elif param_n == "4" and stat != "0":
            test.expect(re.search(r".*\+CGREG: {},\".*\",\".*\",(0|3).*".format(stat), test.dut.at1.last_response))
