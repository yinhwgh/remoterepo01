...
class Test(BaseTest):
    ...
    '''
    TC0104065.001 - TpGEAwithInternetService
    Set different GEA values and check Internet service
    '''
    ...
    def run(test):
        for i in range(1,8):
            test.log.step(f'{i}.1 set AT^SCFG= "Security/GEA","{i}".')
            test.expect(test.dut.at1.send_and_verify(f"AT^SCFG=\"Security/GEA\",\"{i}\"", f".*{i}.*"))
            test.dut.dstl_restart()
            test.expect(test.dut.at1.send_and_verify("AT^SCFG=\"Security/GEA\"", f".*{i}.*"))
            test.dut.dstl_register_to_network()
            test.log.step(f'{i}.2 do some Internet Service like TCP,UDP,HTTP,etc.')
            test.tcp_basic()
    ...
    def tcp_basic(test):
        ...
        test.log.step('4. Send 1450 bytes from client,wait for the feedback from the echo server.')

        test.expect(client_profile.dstl_get_service().dstl_send_sisw_command_and_data(1450))
        test.sleep(2)

        test.log.step('5. Check amount of sent and received data')
        test.expect(test.dut.at1.send_and_verify('at^sisi=1', 'SISI: 1,4,0,{},{},0'.format(1450, 1450)))
