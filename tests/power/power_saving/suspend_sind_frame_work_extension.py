...
#location: Beijing
#TC0104243.001 - SuspendSINDframeworkExtension
...
class sim_handling_during_suspend_basic_functional(BaseTest):
    ...
    def run(test):
        test.log.info("*******************************************************************")
        test.log.info("1. Enable suspendavailable and  suspendready in sind")
        test.log.info("*******************************************************************")
        ...
        test.log.info("*******************************************************************")
        test.log.step("2. Enable SUSPEND mode")
        test.log.info("*******************************************************************")
        test.dut.dstl_enable_suspend_mode(1)

        test.log.info("*******************************************************************")
        test.log.step("3. Enable PSM Timer")
        test.log.info("*******************************************************************")
        ...
        test.expect(test.dut.dstl_check_if_module_is_on_via_dev_board())
        test.dut.at1.wait_for('+CIEV: suspendAvailable,1')
        ...
        test.log.info("*******************************************************************")
        test.expect(test.dut.at1.wait_for(r"{}".format('^SYSRESUME'), timeout= 10+test.dut.dstl_get_actual_periodic_tau() -test.dut.dstl_get_actual_active_time()))
        #test.expect(test.dut.devboard.send_and_verify(r"MC:PWRIND", r'.*MC:   PWRIND: 0','MC:   OK',timeout=2))
        test.expect(test.dut.dstl_check_if_module_is_on_via_dev_board())
        test.sleep(2)
        test.expect(test.dut.at1.wait_for('+CIEV: suspendAvailable,1'))



    def cleanup(test):
        test.log.info("*******************************************************************")
        test.log.step("CleanUp_1: Disable Suspend mode ")
        test.log.info("*******************************************************************")
        test.wakeup_module_via_rts()
        ...
        test.log.info("*******************************************************************")
        test.log.step("CleanUp_2: Disable UART power saving mode ")
        test.log.info("*******************************************************************")
        test.wakeup_module_via_rts()
        test.disable_uart_powersaving()
