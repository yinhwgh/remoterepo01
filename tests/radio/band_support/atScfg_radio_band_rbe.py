...
# location: Berlin
# TC0105169.001
# jira: BOB03-27, BOB-3405
# feature: LM0002647.019, LM0002647.014
...
class Test(BaseTest):
    ...
    Feature:    BOB03-27, BOB-3405 / CR01934
                LM0002647.014/.019: Multi Band/ RAT Selection for Modules Supporting 2G, 3G and 4G RAT
    ...
    def run(test):
        ...
        if test.bBand2G==True:
            ret = test.expect(test.dut.at1.send_and_verify("AT^SCFG=" + "Radio/Band/2G," + "0,,1", ".*\\+CME ERROR: (767|operation failed).*"))
            if ret == False:
                response = test.dut.at1.last_response
                # AT-Spec: 'Switching off all bands returns an error.At least one remaining band must be activated.'
                test.log.error("\n ==> see IPIS100318830, now at least one band has to be still active")
                test.log.error("\n ==> see AT-Spec: 'Switching off all bands returns an error.At least one remaining band must be activated.'")
        ...
        # all bands can not be zero, now without <rbe>  (rbe==0)
        # so we check module behave after restart: it should register to 2G at least
        testSection = "\n --- 2.2 check scfg setting after restart and try to register --- "
        test.log.info(testSection)
        test.dut.dstl_restart()
        ...
        ret = test.expect(test.dut.at1.wait_for(".*\+CREG: 1,.*,.*,[03].*", timeout=25, append=True))    # 0/3: GSM, GSM+EDGE
        if ret == False:
            response = test.dut.at1.last_response
        ...
        test.expect(test.dut.at1.send_and_verify('AT+CREG?', ".*\+CREG: 2,1,.*,.*,[03].*OK.*"))
