...
class Test(BaseTest):
    """
     TC0108309.001 - EBS_AlarmDevice_Maintenance_EF_C
    """
...
def check_network(test, start_step=1):
    if start_step == 1:
        test.log.step(
    ...
    if start_step <= 57:
        ...
        test.socket = SocketProfile(test.dut, srv_profile_id="1", con_id="2", alphabet="1", protocol="tcp",
                                    host=host_address,
                                    port="50000", tcp_mr="10", tcp_ot="6000")
...
def main_process(test):
    ...
    test.log.step('2.Define back end server connection (TCP transparent)')
    ...
    test.log.step('3.Open back end server connection')
    test.expect(test.dut.at1.send_and_verify('AT^SICA=1,1', ".*OK.*"))
    test.expect(test.dut.at1.send_and_verify('AT^SISO=1', ".*OK.*"))

    test.log.step('4.Open transparent access mode')
    test.expect(test.dut.at1.send_and_verify('AT^SIST=1', "CONNECT", wait_for="CONNECT"))

    test.log.step('5.Receive/Send Data.Catch wareshark log to check RTT wheather is 200ms')
    test.dut.at1.send(data_500, end='')
    test.sleep(5)
    test.expect(test.dut.dstl_switch_to_command_mode_by_pluses())

    test.log.step('6.Check service status')
    test.expect(test.dut.at1.send_and_verify('AT^SISI=1', ".*OK.*"))

    test.log.step('7.Close the service')
    test.expect(test.dut.at1.send_and_verify('AT^SISC=1', ".*OK.*"))

    test.log.step('8.CSD callCheck network signal every 30s')
    ...
    test.log.step('8.CSD callCheck network signal every 30s')
    test.expect(test.dut.at1.send_and_verify('ATD911', "connect"))
    test.log.step('9.Send Unstructured Supplementary Service Data (USSD) to the network')
...
def error_handle_b(test):
    test.log.info('Start Error handle b')
    test.expect(test.dut.at1.send_and_verify('at+cfun=1,1'))
    test.expect(test.dut.at1.wait_for('^SYSSTART', 30))
    test.expect(test.socket.dstl_get_parser().dstl_get_service_state(
        at_command=Command.SISI_WRITE) == ServiceState.UP.value)


def error_handle_c(test):
    test.log.info('Start Error handle c')
    test.expect(test.dut.at1.send_and_verify('AT+COPS=2', 'OK'))
    test.expect(test.dut.at1.send_and_verify('AT+COPS=0', expect='.*CREG: 1.*', wait_for='.*CREG: 1.*'))
    check_network(test, 4)
