...
class Test(BaseTest):
    """
     TC0108310.001 - EBS_AlarmDevice_Maintenance_EF_D
    """
...
def main_process(test):
    ...
    test.log.info("******Start Error Handong D******")
    error_handle_d(test)
    ...
    test.dut.at1.send_and_verify('AT+COPS?', '\+COPS: 0,.+,7', ".*OK.*")
    test.log.info("******Start Error Handong D******")
    error_handle_d(test)
...
def error_handle_d(test):
    test.log.info('Start Error handle d')
    test.expect(test.dut.at1.send_and_verify('AT^SISI=1', ".*OK.*"))
    test.expect(test.dut.at1.send_and_verify('AT^SISC=1', ".*OK.*"))
    test.expect(test.dut.at1.send_and_verify('AT^SISI=1', ".*OK.*"))
    check_network(test, 57)
