...
class Test(BaseTest):
    """
     TC0108307.001 - EBS_AlarmDevice_Maintenance_NF
    """
...
def check_network(test):
    ...
    host_address = '114.55.6.216'
    test.socket = SocketProfile(test.dut, srv_profile_id="1", con_id="2", alphabet="1", protocol="tcp",
                                host=host_address, port="50000", tcp_mr="10", tcp_ot="6000")
...
def main_process(test):
    test.log.step('1.Check network')
    check_network(test)

    test.log.step('2.Check service status')
    test.expect(test.socket.dstl_get_parser().dstl_get_service_state(
        at_command=Command.SISI_WRITE) == ServiceState.UP.value)

    test.log.step('3.Close the service')
    test.expect(test.dut.at1.send_and_verify('AT^SISC=1', ".*OK.*"))

    test.log.step('4.Define back end server connection (TCP transparent)')
    ...
    # test.expect(test.dut.at1.send_and_verify('AT^SISS=1,address,"socktcp://dev10.aplnet.dynv6.net:6650;etx;timer=200"',
    #                                          ".*OK.*"))
    test.expect(
        test.dut.at1.send_and_verify('AT^SISS=1,address,"socktcp://114.55.6.216:50000;etx;timer=200"', ".*OK.*"))

    test.log.step('5.Open back end server connection')
    ...
    test.log.step('6.Open transparent access mode')
    test.expect(test.dut.at1.send_and_verify('AT^SIST=1', "CONNECT", wait_for="CONNECT"))

    test.log.step('7.Receive/Send Data.Catch wareshark log to check RTT wheather is 200ms')
    # test.dut.at1.send(data_500, end='')
    for i in range(1, 100):
        test.log.info(f"This is {i} loop in transparent.")
        start_time = time.time()
        test.dut.at1.send('65dca26e', end='')
        test.expect(test.dut.at1.wait_for('65dca26e'), critical=True)
        end_time = time.time()
    ...
    test.log.info('echo_times: ' + " ".join(map(str, echo_times)))

    test.log.step('8.Check service status')
    test.expect(test.dut.at1.send_and_verify('AT^SISI=1', ".*OK.*"))

    test.log.step('9.Close the service')
    test.expect(test.dut.at1.send_and_verify('AT^SISC=1', ".*OK.*"))

    test.log.step('10.CSD callCheck network signal every 30s')
    test.expect(test.dut.at1.send_and_verify('ATD911', "connect"))

    test.log.step('11.Send Unstructured Supplementary Service Data (USSD) to the network')
    test.expect(test.dut.at1.send_and_verify('AT+CUSD=1,"*100#",15', ".*OK.*"))
