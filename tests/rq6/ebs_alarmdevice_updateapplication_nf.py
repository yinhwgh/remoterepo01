...
class Test(BaseTest):
    ...
    """
     TC0108058.001 - EBS_AlarmDevice_UpdateApplication_NF
    """
...
def main_process(test):
    ...
    check_network(test)
    test.log.step('1. Check service status')
    test.expect(test.socket.dstl_get_parser().dstl_get_service_state(
        at_command=Command.SISI_WRITE) == ServiceState.UP.value)
    test.log.step('2. Check software update download')
    test.expect(test.socket.dstl_get_service().dstl_send_sisw_command_and_data
                (51, repetitions=1))
    test.log.step('3.Read and save to application memory new software.')
    test.sleep(60)
    test.expect(test.socket.dstl_get_service().dstl_read_all_data
                (req_read_length=1500))
    test.log.step('.Go to "Check network" step 81 (Application will initiate restart cycle when updated).'
                  ' TC0108051.001 ')
    for i in range(1, 5):
        test.expect(test.dut.at1.send_and_verify('AT+CREG?', ".*OK.*"))
        test.expect(test.dut.at1.send_and_verify('AT+CRSM=242', ".*OK.*"))
