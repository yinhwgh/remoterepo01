...
def main_process(test):
    ...
    test.expect(test.dut.at1.send_and_verify("AT+CGDCONT=1,IPV4V6,internet4g.gdsp", ".*OK.*"))
    ...
    test.log.info("alternative flow: remove sim card.")
    test.expect(test.dut.devboard.send_and_verify("MC:CCIN=1"))
    test.sleep(5)
    test.expect(socket_service.dstl_get_service().dstl_open_service_profile(expected=".*ERROR.*"))
    test.log.info("alternative flow: insert sim card.")
    test.expect(test.dut.devboard.send_and_verify("MC:CCIN=0"))
    test.sleep(10)
    dstl_enter_pin(test.dut)
    test.sleep(5)
    test.log.info("alternative flow: attempts to close the connection.")
    test.expect(socket_service.dstl_get_service().dstl_close_service_profile())
    test.log.info("alternative flow: Opens the internet service again.")
    test.expect(test.dut.at1.send_and_verify("AT^SICA=1,1", ".*OK.*"))
    test.sleep(10)
    # ssl_server_thread = test.thread(test.ssl_server.dstl_run_ssl_server)
    # test.sleep(10)
    ...
    test.log.step(
        '5. Reads the file which is about the size of 100k-2Mbyte in 1500 Byte chunks stores it temporarily on the application processor memory and checks the connection status with AT^SISI=0 in between.')
    for i in range(10):
