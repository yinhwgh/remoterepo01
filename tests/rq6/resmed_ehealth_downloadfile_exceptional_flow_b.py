...
class Test(BaseTest):
    """
     TC0107800.001-Resmed_eHealth_DownloadFile_ExceptionalFlow_B
    """
...
def EF_B_HW_restart(test, checkstep, restart):
    test.log.info("run after at+cfun=1,1 is failed.")
...
def main_process(test, restart=0):
    ...
    server_send_data = '!@#$%^&*()'+dstl_generate_data(1490)
    ...
    if EF_B_HW_restart(test, 1, restart):
        return True
    ...
    if EF_B_HW_restart(test, 2, restart):
        return True
    ...
    if EF_B_HW_restart(test, 3, restart):
        return True
    ...
    for i in range(10):
        ...
        test.sleep(1)
    if EF_B_HW_restart(test, 4, restart):
        return True
    ...
    if EF_B_HW_restart(test, 5, restart):
        return True
    ...
    if EF_B_HW_restart(test, 6, restart):
        return True
    ...
    if EF_B_HW_restart(test, 7, restart):
        return True
