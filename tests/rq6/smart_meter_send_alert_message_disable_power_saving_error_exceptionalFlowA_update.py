...
# location: Beijing
# TC0107137.001 - SmartMeter_sendAlertMessage_disable_power_saving_error_exceptionalFlowA_update
# Hints:
...
class Test(BaseTest):
    ...
    def run(test):
        ...
        send_alert_message_disable_power_saving_error_exceptional_flowA_update(test)

    def cleanup(test):
        pass

def send_alert_message_disable_power_saving_error_exceptional_flowA_update(test):
    global power_save_wakeup, power_save_period, tamperingAttempt_status, glitch_status, power_loss, ipVersion
    ...
    power_loss = False
    ipVersion = "4"
    test.log.info('*** send_alert_message_disable_power_saving_error_exceptional_flowA_update begin ***')
    ...
    test.dut.dstl_collect_result('NF-05:  Wake up the module.',
                                 smart_meter_init_module_normal_flow.retry(test, smart_meter_send_alert_message_normal_flow_update.wake_up_module,
                                                                           ...
                                                                           3))
    time.sleep(3)
    power_save_period = 601
    test.log.step('NF-06: disables power saving.--- Trigger error')
    test.log.step('NF-07: Shut down the module via at^smso.')
    test.log.step('NF-08: Wait a security time about 5 seconds.')
    test.log.step('NF-09: Boot up the module via the ignition line and re-initialized module.')
    test.dut.dstl_collect_result('NF-06: disables power saving.\n'
                                 'NF-07: Shut down the module via at^smso.\n'
                                 'NF-08: Wait a security time about 5 seconds.\n'
                                 'NF-09: Boot up the module via the ignition line and re-initialized module.',
                                 smart_meter_init_module_normal_flow.retry(test, disable_power_saving_mode,
                                                                           AF_A_restart,
                                                                           3))
    power_save_period = 52
    test.dut.at1.reconfigure({"rtscts": False})
    test.log.step('NF-10: Repeat step1-6.--- NF-01: Check for listener mode.')
    smart_meter_init_module_normal_flow.set_listener_mode(False)
    if smart_meter_init_module_normal_flow.check_listener_mode(test):
        if smart_meter_init_module_normal_flow.check_dcd_line(test) is True:
            return
            ...
            smart_meter_init_module_normal_flow.close_listener(test)
            test.dut.dstl_collect_result('NF-10: Repeat step1-6.--- NF-01: Check for listener mode', False)
    test.dut.dstl_collect_result('NF-10: Repeat step1-6.--- NF-01: Check for listener mode', True)

    test.log.step('NF-10: Repeat step1-6.--- NF-02: Check if tampering attempt.')
    tamperingAttempt_status = False
    test.dut.dstl_collect_result('NF-10: Repeat step1-6.--- NF-02: Check if tampering attempt.', True)

    test.log.step('NF-10: Repeat step1-6.--- NF-03: Send the module to sleep.')
    enable_power_saving_mode(test)
    test.expect(test.dut.at1.send_and_verify("AT\Q 0", ".*OK|.*0"))
    test.expect(test.dut.at1.send_and_verify('AT^SCFG="tcp/withurcs","on"', ".*OK|.*0"))
    test.dut.dstl_collect_result('NF-10: Repeat step1-6.--- NF-03: Send the module to sleep.',
                                 smart_meter_init_module_normal_flow.retry(test, check_enter_sleep,
                                              smart_meter_init_module_normal_flow.AF_A_restart, 3))

    test.log.step('NF-10: Repeat step1-6.--- NF-04:  Check if really power loss or only a glitch for 9 seconds.')
    time.sleep(9)
    test.dut.dstl_collect_result('NF-10: Repeat step1-6.--- NF-04:  Check if really power loss or only a glitch for 9 seconds.', True)

    test.log.step('NF-10: Repeat step1-6.--- NF-05:  Wake up the module.')
    test.dut.dstl_collect_result('NF-10: Repeat step1-6.--- NF-05:  Wake up the module.',
                                 smart_meter_init_module_normal_flow.retry(test, smart_meter_send_alert_message_normal_flow_update.wake_up_module,
                                                                           smart_meter_init_module_normal_flow.AF_A_restart,
                                                                           3))

    test.log.step('NF-10: Repeat step1-6.--- NF-06: disables power saving.')
    test.dut.dstl_collect_result('NF-10: Repeat step1-6.--- NF-06: disables power saving.',
                                 smart_meter_init_module_normal_flow.retry(test, disable_power_saving_mode,
                                                                           smart_meter_init_module_normal_flow.AF_A_restart,
                                                                           3))
    ...
    test.log.info('*** send_alert_message_disable_power_saving_error_exceptional_flowA_update end ***')
...
def AF_A_restart(test):
    ...
    test.expect(test.dut.at1.send_and_verify(
         'AT^SCFG="MEopMode/PwrSave","disabled","52","50"', ".*OK.*|.*0.*"))
    restart_cuunter = restart_cuunter + 1
    test.log.step("[precondition] restore")
    smart_meter_init_module_normal_flow.mc_insert_sim(test)
    smart_meter_init_module_normal_flow.mc_register_network(test)
    test.log.step("*****Shut down module *****")
    test.dut.at1.send_and_verify("AT^SMSO", smart_meter_init_module_normal_flow.at_expect_response_1)
    time.sleep(5)

    test.log.step("*****Re-init module *****")
    test.expect(smart_meter_init_module_normal_flow.uc_init_module(test, 2))
