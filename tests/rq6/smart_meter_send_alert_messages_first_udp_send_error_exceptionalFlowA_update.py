...
# location: Beijing
# TC0107167.001 - SmartMeter_sendAlertMessage_first_udp_send_error_exceptionalFlowA_update
# Hints:
...
class Test(BaseTest):
    ...
    def run(test):
        ...
        send_alert_message_first_udp_send_error_exceptional_flowA_update(test)

    def cleanup(test):
        pass

def send_alert_message_first_udp_send_error_exceptional_flowA_update(test):
    ...
    power_loss = False
    ipVersion = "4"
    test.log.info('*** send_alert_message_first_udp_send_error_exceptional_flowA_update begin ***')
    ...
    test.log.step('NF-09: Open UDP socket and send the alert as UDP packet.--- Trigger error')
    test.log.step('NF-10: Shut down the module via at^smso.')
    ...
    test.dut.dstl_collect_result('NF-09: Open UDP socket and send the alert as UDP packet.\n'
                                 'NF-10: Shut down the module via at^smso.\n'
                                 'NF-11: Wait a security time about 5 seconds.\n'
                                 'NF-12: Boot up the module via the ignition line and re-initialized module.',
                                 smart_meter_init_module_normal_flow.retry(test, open_udp_socket_and_send_data,
                                                                           smart_meter_init_module_normal_flow.AF_A_restart,
                                                                           3))

    test.dut.at1.reconfigure({"rtscts": False})
    test.log.step('NF-13: Repeat step1-9 --- NF-01: Check for listener mode.')
    smart_meter_init_module_normal_flow.set_listener_mode(False)
    if smart_meter_init_module_normal_flow.check_listener_mode(test):
        if smart_meter_init_module_normal_flow.check_dcd_line(test) is True:
            return
            ...
            smart_meter_init_module_normal_flow.close_listener(test)
            test.dut.dstl_collect_result('NF-13: Repeat step1-9 --- NF-01: Check for listener mode', False)
    test.dut.dstl_collect_result('NF-13: Repeat step1-9 --- NF-01: Check for listener mode', True)

    test.log.step('NF-13: Repeat step1-9 --- NF-02: Check if tampering attempt.')
    tamperingAttempt_status = False
    test.dut.dstl_collect_result('NF-13: Repeat step1-9 --- NF-02: Check if tampering attempt.', True)

    test.log.step('NF-13: Repeat step1-9 --- NF-03: Send the module to sleep.')
    enable_power_saving_mode(test)
    test.expect(test.dut.at1.send_and_verify("AT\Q 0", ".*OK|.*0"))
    test.expect(test.dut.at1.send_and_verify('AT^SCFG="tcp/withurcs","on"', ".*OK|.*0"))
    test.dut.dstl_collect_result('NF-13: Repeat step1-9 --- NF-03: Send the module to sleep.',
                                 smart_meter_init_module_normal_flow.retry(test, check_enter_sleep,
                                                                           smart_meter_init_module_normal_flow.AF_A_restart,
                                                                           3))

    test.log.step('NF-13: Repeat step1-9 --- NF-04:  Check if really power loss or only a glitch for 9 seconds.')
    time.sleep(9)
    test.dut.dstl_collect_result('NF-13: Repeat step1-9 --- NF-04:  Check if really power loss or only a glitch for 9 seconds.', True)

    test.log.step('NF-13: Repeat step1-9 --- NF-05:  Wake up the module.')
    test.dut.dstl_collect_result('NF-13: Repeat step1-9 --- NF-05:  Wake up the module',
                                 smart_meter_init_module_normal_flow.retry(test,
                                                                           smart_meter_send_alert_message_normal_flow_update.wake_up_module,
                                                                           smart_meter_init_module_normal_flow.AF_A_restart,
                                                                           3))

    test.log.step('NF-13: Repeat step1-9 --- NF-06: disables power saving.')
    test.dut.dstl_collect_result('NF-13: Repeat step1-9 --- NF-06: disables power saving.',
                                 smart_meter_init_module_normal_flow.retry(test, disable_power_saving_mode,
                                                                           smart_meter_init_module_normal_flow.AF_A_restart,
                                                                           3))

    test.log.step('NF-13: Repeat step1-9 --- NF-07: Check if only a glitch.')
    glitch_status = False
    test.dut.dstl_collect_result('NF-13: Repeat step1-9 --- NF-07: Check if only a glitch.', True)

    test.log.step('NF-13: Repeat step1-9 --- NF-08: Configure the first UDP socket.')
    test.dut.dstl_collect_result('NF-13: Repeat step1-9 --- NF-08: Configure the first UDP socket.',
                                 smart_meter_init_module_normal_flow.retry(test,
                                                                           configure_udp1_socket,
                                                                           smart_meter_init_module_normal_flow.AF_A_restart,
                                                                           3))

    test.log.step('NF-13: Repeat step1-9 --- NF-09: Open UDP socket and send the alert as UDP packet.')
    test.dut.dstl_collect_result('NF-13: Repeat step1-9 --- NF-09: Open UDP socket and send the alert as UDP packet',
                                 smart_meter_init_module_normal_flow.retry(test, open_udp_socket_and_send_data,
                                                                          smart_meter_init_module_normal_flow.AF_A_restart,3))
    ...
    test.log.info('*** send_alert_message_first_udp_send_error_exceptional_flowA_update end ***')
