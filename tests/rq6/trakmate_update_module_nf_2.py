...
up_grade=True    #if degrade , set up_grade=False, else True
...
def whole_flow(test, upgrade,abort=False):
    ...
    if check_sw(test, upgrade):
        ...
        #NF_30_open_service_3,NF_32_check_sisw_urc
        send_data.whole_flow(test,send_data.step_list[13:15])
        test.log.step('5.Send message to server for the alert notification with MQTT')
        ...
        #test.expect(test.dut.at1.send_and_verify_retry('AT^SISI=3', 'SISI: 3,4,0,30'))
...
def trigger_swup(test, abort=False):
    test.expect(
