...
class Test(BaseTest):
    ...
    Intention:
    Testing if DTLS connection can be established when on module and on server different DTLS
    versions can be accepted.

    description:
    1. Run OpenSSL server using DTLSv1:
    s_server -accept 50303 -state -naccept 1 -cert micCert.pem -certform pem -key mic.pem
    -keyform pem -dtls1 -brief
