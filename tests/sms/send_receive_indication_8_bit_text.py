...
from dstl.sms.configure_sms_text_mode_parameters import dstl_set_sms_text_mode_parameters, \
    dstl_show_sms_text_mode_parameters, dstl_configure_sms_event_reporting, \
    dstl_read_sms_event_reporting_configuration, dstl_enable_sms_class_0_display, \
    dstl_confirm_acknowledgement_new_sms_deliver
...
class Test(BaseTest):
    """
    TC0011139.001   SendReceiveIndication8bitText
    ...
    In case 1-5 no indication is shown, in other +CMTI: information appears
        (in case of SMS class 0 +CMT:).
    ...
    def execute_scenario_for_one_class(test, sender, receiver, mode, mt, sms_class):
        ...
        test.log.info(f"===== set CSMP on sender module for SMS class {sms_class} "
                      f"and send SMS =====")
        test.expect(dstl_set_sms_text_mode_parameters(sender, "17", "167", "0", dcs))
        test.expect(dstl_send_sms_message(sender, receiver.sim.int_voice_nr, msg_content,
    ...
    def verify_sms(test, module, mode, mt, sms_class):
        ...
        if mode == test.MODE_0 or mt == test.MT_0:
            ...
            test.expect(dstl_list_sms_messages_from_preferred_memory(module, "ALL"))
            if mt == test.MT_1 and sms_class == test.SMS_CLASS_0:
                ...
                test.log.info(f"Expected phrase : .*{msg_content}.*")
                test.expect(re.search(f".*{msg_content}.*", module.at1.last_response))
            if sms_class == test.SMS_CLASS_2:
            ...
            if mt == test.MT_1 and sms_class == test.SMS_CLASS_0:
                ...
                test.expect(dstl_check_urc(module, f".*CMT:.*{msg_content}.*",
                ...
                test.expect(dstl_confirm_acknowledgement_new_sms_deliver(module,
