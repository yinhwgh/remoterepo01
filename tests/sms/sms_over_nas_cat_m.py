...
class Test(BaseTest):
    """TC0104296.001    SmsOverNasCatM
    The goal of this TC is to check if MO and MT SMS works correctly over NAS in Cat-M.

    Description:
    1. 1. Attach DUT(Cat-M) and REM(if supported NAS or Cat-M NAS)
    ...
    def run(test):
        test.log.h2("Starting TC0104296.001 SmsOverNasCatM")
        test.log.step("Step 1. Attach DUT(Cat-M) and REM(if supported NAS or Cat-M NAS)")
    ...
    def prepare_module(test, module, text):
        test.log.info(text)
        dstl_set_character_set(module, 'GSM')
    ...
    def check_ims_for_sms_and_attach_to_lte(test, module, ims_support, sms_domain_support):
        ...
        if sms_domain_support:
            ...
            test.log.info("*** SMS/4GPref setting NOT supported by module ***")
        test.log.info("===== Check registration status =====")
        test.expect(module.at1.send_and_verify("AT+COPS?", ".*OK.*"))
        if module is test.dut:
            regex_lte_attach = r".*COPS:.*,.*,.*,(7).*"
            info_attach = "*** Module registered to CatM ***"
        else:
            regex_lte_attach = r".*COPS:.*,.*,.*,(7|9).*"
            info_attach = "*** Module registered to LTE ***"
        if re.search(regex_lte_attach, module.at1.last_response):
            if module is test.dut:
                test.dut.at1.send_and_verify('AT^SMONI', '.*SMONI: Cat.M1,.*,NOCONN.*')
            test.log.info(info_attach)
        else:
            test.log.info("*** Register module to LTE ***")
            test.expect(dstl_register_to_lte(module))
            if module is test.dut:
                test.expect(module.at1.send_and_verify("AT+COPS?", r".*COPS:.*,.*,.*,(7).*OK.*"))
                test.dut.at1.send_and_verify('AT^SMONI', '.*SMONI: Cat.M1,.*,NOCONN.*')
                test.log.info(info_attach)
