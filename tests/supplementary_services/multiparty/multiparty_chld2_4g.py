...
- R1 accept call
- R2 call DUT
- DUT use chld=2 (put active call on hold)
- DUT toggle between calls via chld=2
- DUT start conference via CHLD=3
- terminate all calls via chup

Test 10 - chld=2
- DUT call R1
- R1 accept call
- DUT use chld=2 to put active call on hold
- DUT call R2
- DUT end call to R2
- DUT use chld=2 to activate the call again
- teminate all calls
'''
...
class multiparty_chld2_4g(BaseTest):
    ...
    def run(test):
        ...
        if not call_waiting:
            ...
            test.log.step('Step 3.0: Multiparty Test 9 #\n'
                          '- DUT call A and A accept call\n'
                          '- B call DUT\n'
                          '- DUT use chld=2 (put active call on hold)\n'
                          '- DUT toggle between calls via chld=2\n '
            ...
            incoming_number = test.dut.dstl_strip_number(test.dut.at1.last_response)
            call_array_r2 = test.dut.dstl_clcc_slcc_check(incoming_number, module_has_slcc)
            test.expect(test.dut.dstl_check_call(call_array_r2, "5", incoming_number, module_has_slcc, False))

            test.log.step ('Step 3.3: Place active calls on hold and accept the waiting call')
            # ==============================================================
            test.expect(test.dut.at1.send_and_verify("at+chld=2", "OK"))
            test.sleep(wait_time)

            test.log.step ('Step 3.4: toggle 10 times between the calls and check the call IDs')
            # ==============================================================
            i=1
            i_end = 10 # number of loops
            while i<i_end+1:
                loop_info = 'Step 3.4.' + str(i) + ' : loop: ' + str(i) + ' of ' + str(i_end) + ' loops'
                test.log.step(loop_info)
                call_array_r1 = test.dut.dstl_clcc_slcc_check(tln_r1_nat, module_has_slcc)
                call_array_r2 = test.dut.dstl_clcc_slcc_check(incoming_number, module_has_slcc)
                test.expect(test.dut.dstl_requested_call_state("active", call_array_r2, module_has_slcc))
                test.expect(test.dut.dstl_requested_call_state("held", call_array_r1, module_has_slcc))
                #
                # test.expect(test.dut.dstl_check_call(call_array_r2, "0", incoming_number, module_has_slcc, False))
                # test.expect(test.dut.dstl_check_call(call_array_r1, "1", tln_r1_nat, module_has_slcc, False))
                ...
                test.expect(test.dut.dstl_requested_call_state("held", call_array_r2, module_has_slcc))
                test.expect(test.dut.dstl_requested_call_state("active", call_array_r1, module_has_slcc))

                # test.expect(test.dut.dstl_check_call(call_array_r2, "1", incoming_number, module_has_slcc, False))
                # test.expect(test.dut.dstl_check_call(call_array_r1, "0", tln_r1_nat, module_has_slcc, False))
            ...
            test.log.step ('Step 3.7: Check the call IDs, call state after multiparty starts')
            # ==============================================================
            test.expect(test.dut.dstl_compare_call_ids (call_array_r1, multi_call_array_r1, module_has_slcc))
            test.expect(test.dut.dstl_compare_call_ids (call_array_r2, multi_call_array_r2, module_has_slcc))
            ...
            test.log.step ('Step 3.8: terminate the calls via chup')
            # ==============================================================
            test.expect(test.dut.at1.send_and_verify(tln_dut_hang_up_call, "OK"))
            test.sleep(wait_time)
            test.sleep(wait_time)
        ...
        # ==============================================================
        # ==============================================================
        test.log.step('Step 4.0: Multiparty Test 10 # \n'
                      '- DUT call A - A accept call \n'
                      '- DUT use chld=2 to put active call on hold \n'
                      '- DUT call B '
                      '- DUT end call to B \n'
                      '- DUT use chld=2 to activate the first call again \n'
                      '- teminate all calls')

        test.log.step ('Step 4.1: DUT call R1 and R1 accept the call')
        # ==============================================================
        test.expect(test.dut.dstl_voice_call_by_number(test.r1, tln_r1_nat))
        test.sleep(wait_time)
        call_array_r1 = test.dut.dstl_clcc_slcc_check(tln_r1_nat, module_has_slcc)
        ...
        test.log.step ('Step 4.2: DUT use chld=2 to put active call on hold')
        # ==============================================================
        test.expect(test.dut.at1.send_and_verify("at+chld=2", "OK"))
        call_array_r1 = test.dut.dstl_clcc_slcc_check(tln_r1_nat, module_has_slcc)
        test.expect(test.dut.dstl_check_call(call_array_r1, "1", tln_r1_nat, module_has_slcc, False))

        test.log.step ('Step 4.3: DUT call B and B accept the call')
        # ==============================================================
        test.expect(test.dut.dstl_voice_call_by_number(test.r2, tln_r2_nat))
        test.sleep(wait_time)
        call_array_r2 = test.dut.dstl_clcc_slcc_check(tln_r2_nat, module_has_slcc)
        multi_call_array_r1 = test.dut.dstl_clcc_slcc_check(tln_r1_nat, module_has_slcc)
        test.expect(test.dut.dstl_check_call(call_array_r2, "0", tln_r2_nat, module_has_slcc, False))

        test.log.step ('Step 4.4: check the call state from A')
        # ==============================================================
        test.expect(test.dut.dstl_check_call(multi_call_array_r1, "1", tln_r1_nat, module_has_slcc, False))
        test.expect(test.dut.dstl_compare_call_ids(call_array_r1, multi_call_array_r1, module_has_slcc))
        test.sleep(wait_time)

        test.log.step ('Step 4.5: B end call to DUT')
        # ==============================================================
        test.expect(test.r2.at1.send_and_verify(tln_r2_hang_up_call, "OK"))
        test.sleep(wait_time)

        test.log.step ('Step 4.6: DUT use chld=2 to activate the first call again')
        # ==============================================================
        test.expect(test.dut.at1.send_and_verify("at+chld=2", "OK"))

        test.log.step('Step 4.7: check the call ID from the activ call with the call ID from the start')
        # ==============================================================
        multi_call_array_r1 = test.dut.dstl_clcc_slcc_check(tln_r1_nat, module_has_slcc)
        test.expect(test.dut.dstl_compare_call_ids(call_array_r1, multi_call_array_r1, module_has_slcc))
        test.expect(test.dut.dstl_check_call(multi_call_array_r1, "0", tln_r1_nat, module_has_slcc, False))
        test.sleep(wait_time)

        test.log.step('Step 4.8: terminate the calls via chup')
        # ==============================================================
        test.expect(test.dut.at1.send_and_verify(tln_dut_hang_up_call, "OK"))
        test.sleep(wait_time)
        test.dut.dstl_check_call_state_all(test.r1, test.r2, module_has_slcc, tln_dut_hang_up_call, tln_r1_hang_up_call,
        ...
        # ==============================================================
        test.log.step('Step 5.0: Multiparty Test 2 #\n'
                      '- DUT call A and A accept call\n'
                      '- DUT call B and B accept call\n'
                      '- DUT start conference call\n'
                      '- terminate all calls via chup')

        test.log.step ('Step 5.1: DUT call R1 and R1 accept the call')
        # ==============================================================
        test.expect(test.dut.dstl_voice_call_by_number(test.r1, tln_r1_nat))
        ...
        test.log.step ('Step 5.2: DUT call R2 call and DUT accept the call')
        # ==============================================================
        #test.expect(test.r2.at1.send_and_verify("atd" + str(tln_dut_nat) + ";" , "OK"))
        test.expect(test.dut.dstl_voice_call_by_number(test.r2, tln_r2_nat))
        test.sleep(wait_time)
        call_array_r2 = test.dut.dstl_clcc_slcc_check(tln_r2_nat, module_has_slcc)
        test.expect(test.dut.at1.send_and_verify("at+chld=2", "OK"))
        ...
        test.log.step ('Step 5.3: check Call states')
        # ==============================================================
        # check the call states and IDs
        # call to R1 must be active
        # test.expect(test.dut.dstl_requested_call_state("held", call_array_r1, module_has_slcc))
        ...
        test.log.step('Step 5.4: DUT toggle the call with at+chld=2')
        # ==================================================================
        test.expect(test.dut.at1.send_and_verify("at+chld=2", "OK"))
        test.sleep(wait_time)
        multi_call_array_r1 = test.dut.dstl_clcc_slcc_check(tln_r1_nat, module_has_slcc)
        ...
        test.log.step('Step 5.5: DUT start conference call with at+chld=3')
        # ==================================================================
        test.expect(test.dut.at1.send_and_verify("at+chld=3", "OK"))
        test.sleep(wait_time)
        test.expect(test.dut.at1.send_and_verify("at+clcc"))

        test.log.step ('Step 5.6: Compare the call IDs before and after the start of Mulitparty')
        # ==============================================================
        multi_call_array_r1 = test.dut.dstl_clcc_slcc_check (tln_r1_nat, module_has_slcc)
        multi_call_array_r2 = test.dut.dstl_clcc_slcc_check (tln_r2_nat, module_has_slcc)
        ...
        test.log.step ('Step 5.7: terminate the calls via chup')
        # ==============================================================
        test.expect(test.dut.at1.send_and_verify(tln_dut_hang_up_call, "OK"))
        test.sleep(wait_time)
        test.sleep(wait_time)
