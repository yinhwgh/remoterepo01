...
class Test(BaseTest):
    ...
    def run(test):
        ...
        test.expect(test.dut.at1.send_and_verify("AT^SIND?", ".*\^SIND: euiccid,0.*OK.*"))
        test.expect(test.dut.at1.send_and_verify("AT^SIND=euiccid,2", "\^SIND: euiccid,0.*OK.*"))
        ...
        test.log.info('**** Wating for "euiccid" URC ****')
        test.expect("+CIEV: euiccid," in test.dut.at1.last_response)
        ...
        t2 = threading.Thread(target = test.sind_set_euiccid)
        t1.start()
        t2.start()
        t1.join()
        ...
        t4 = threading.Thread(target=test.sind_set_euiccid)
        t3.start()
        t4.start()
        t3.join()
    ...
    def sind_set_euiccid(test):
        time.sleep(2)
        test.expect(test.dut.at1.send_and_verify("AT^SIND=euiccid,0", "\^SIND: euiccid,0.*OK.*"))
        test.expect(test.dut.at1.send_and_verify("AT^SIND=euiccid,1", "\^SIND: euiccid,1.*OK.*"))
