...
# location: Beijing
# TC0104106.001 - TpSATSendDtmf_AR
...
class TpSATSendDtmf(BaseTest):
    ...
    def run(test):
        senddtmf_applet_sstk(test,'MenuApplet')
...
def senddtmf_applet_sstk(test,trigger_mode):
    # *** set Proactive_Command and Terminal_Response
    proactive_cmd = 'D00D0103021400020281832C022143'
    if (test.dut.project == 'VIPER'):
        terminal_response = '010302140002028281030100'
    else:
        terminal_response = '01030214000202828103022004'
    ...
    if trigger_mode == 'MenuApplet':
        ...
        test.log.info("***** Update the EF record #2 with SEND DTMF proactive command ****")
    ...
    test.log.info("**** Setup a call to any destination number and watch DTMFs ****")
    #        test.dut.dstl_voice_call_by_number(01723402148)
    test.expect(test.dut.at1.send_and_verify('ATD01723402148;', ".*OK.*", timeout=30))

    if trigger_mode == 'MenuApplet':
        test.log.info("**** [SSTA=0]: Trigger execution of record #2 Send DTMF ****")
